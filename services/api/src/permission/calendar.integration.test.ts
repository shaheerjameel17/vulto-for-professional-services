...
async function createEntity(
  ...
  const result = await w.apply("owner", "entity.create", {
    ...
    jurisdiction,
    default_currency:
      jurisdiction === "UK" ? "GBP" : jurisdiction === "US" ? "USD" : "PKR",
  });
...
describe("VRS-F004 — Working Calendar and Working Patterns", () => {
  ...
  it("versions calendars, copies only Active holidays, and freezes historical periods", async () => {
    ...
    const activeHoliday = await w.apply("hr", "holiday.add", {
      calendar_id: calendarId,
      fields: { name: "Active", date: "2026-05-01", holiday_type: "Public" },
    });
    const canceledHoliday = await w.apply("hr", "holiday.add", {
      calendar_id: calendarId,
      fields: { name: "Canceled", date: "2026-05-02", holiday_type: "Company" },
    });
    await w.apply("hr", "holiday.cancel", {
      holiday_id: (canceledHoliday.result as { holiday_id: string }).holiday_id,
      expected_version: 1,
    });
    const periods = [
      {
    ...
    const copied = (
      await db.transaction((tx) =>
        getNodes(tx, w.workspaceId, { nodeType: "Holiday", lifecycleStatus: "Active" }),
      )
    ).filter((h) => h.record["calendar_id"] === firstId);
    expect(copied).toHaveLength(1);
    expect(copied[0]?.record["name"]).toBe("Active");
    expect(activeHoliday.status).toBe("applied");
    const activeHolidayId = (activeHoliday.result as { holiday_id: string }).holiday_id;
    expect(
      (
    ...
    expect(
      (
        await w.apply("hr", "holiday.confirm", {
          holiday_id: activeHolidayId,
    ...
    expect(
      (await db.transaction((tx) => getNode(tx, w.workspaceId, secondId)))?.record[
        "reduced_hours_periods"
      ],
    ).toEqual(periods);
    const historicalFirst = await db.transaction((tx) =>
      resolveCalendarForEntity(tx, w.workspaceId, entityId, "2026-01-15"),
    );
    expect(historicalFirst?.nodeId).toBe(firstId);
    expect(historicalFirst?.record["reduced_hours_periods"]).toEqual(periods);
    expect(
      (
        await db.transaction((tx) =>
          resolveCalendarForEntity(tx, w.workspaceId, entityId, "2026-01-05"),
        )
      )?.nodeId,
    ).toBe(calendarId);
  });
  ...
  it("enforces pattern versions, shared boundaries, and inherited weekdays", async () => {
    ...
    const first = await w.apply("hr", "pattern.set", {
      employee_id: employeeId,
      working_week: [{ day: 3, is_working: false, hours: 0 }],
      effective_from: "2026-03-01",
    });
    expect(first.status, JSON.stringify(first)).toBe("applied");
    const firstPatternId = (first.result as { pattern_id: string }).pattern_id;
    const patternEdges = await db.transaction((tx) =>
      incoming(tx, w.workspaceId, employeeId, "pattern_for"),
    );
    expect(patternEdges.some((edge) => edge.fromNodeId === firstPatternId)).toBe(true);
    expect(
      await db.transaction((tx) =>
        hoursOn(tx, w.workspaceId, employeeId, "2026-03-02", allowAll),
      ),
    ).toBe(8);
    expect(
      await db.transaction((tx) =>
        hoursOn(tx, w.workspaceId, employeeId, "2026-03-04", allowAll),
      ),
    ).toBe(0);
    expect(
      (
        await w.apply("hr", "pattern.set", {
          employee_id: employeeId,
          working_week: [],
    ...
    expect(
      await db.transaction((tx) => getNode(tx, w.workspaceId, firstPatternId)),
    ).toMatchObject({
    ...
    expect(
      (
        await w.apply("hr", "pattern.clear", {
          employee_id: employeeId,
  ...
  it("resolves AE, PK, UK pattern, regional holiday and Ramadan cases through one API", async () => {
    ...
    expect(
      await db.transaction((tx) =>
        hoursOn(tx, w.workspaceId, aeEmployee, "2026-01-09", allowAll),
      ),
    ).toBe(0);
    expect(
      await db.transaction((tx) =>
        nextWorkingDay(tx, w.workspaceId, aeEmployee, "2026-01-08", 1, allowAll),
      ),
    ).toBe("2026-01-11");
    expect(
      await db.transaction((tx) =>
        addWorkingDays(tx, w.workspaceId, aeEmployee, "2026-01-08", 2, allowAll),
      ),
    ).toBe("2026-01-12");
    expect(
      await db.transaction((tx) =>
    ...
    ).toEqual({ days: 5.5, hours: 44 });
    await w.apply("hr", "pattern.set", {
      employee_id: ukEmployee,
      working_week: [{ day: 1, is_working: false, hours: 0 }],
      effective_from: "2026-03-01",
    });
    expect(
      await db.transaction((tx) =>
        hoursOn(tx, w.workspaceId, ukEmployee, "2026-02-23", allowAll),
      ),
    ).toBe(8);
    expect(
      await db.transaction((tx) =>
        hoursOn(tx, w.workspaceId, ukEmployee, "2026-03-02", allowAll),
      ),
    ).toBe(0);
    await w.apply("hr", "holiday.add", {
      calendar_id: pk.calendarId,
      fields: {
        name: "Sindh Day",
        date: "2026-03-23",
        holiday_type: "Regional",
        applies_to_locations: ["Karachi"],
      },
    });
    expect(
      await db.transaction((tx) =>
        hoursOn(tx, w.workspaceId, karachi, "2026-03-23", allowAll),
      ),
    ).toBe(0);
    expect(
      await db.transaction((tx) =>
        hoursOn(tx, w.workspaceId, lahore, "2026-03-23", allowAll),
      ),
    ).toBe(8);
    await w.apply("hr", "holiday.add", {
      calendar_id: pk.calendarId,
    ...
    expect(
      await db.transaction((tx) =>
        hoursOn(tx, w.workspaceId, karachi, "2026-03-24", allowAll),
      ),
    ).toBe(4);
    const reduced = [
      {
    ...
    const updated = await w.apply(
      "hr",
      "calendar.update",
      {
        calendar_id: ae.calendarId,
    ...
    expect(
      await db.transaction((tx) =>
        hoursOn(tx, w.workspaceId, aeEmployee, "2026-02-23", allowAll),
      ),
    ).toBe(6);
  });
  ...
  it("confirms live provisional holidays, preserves estimates, and applies stale checks first", async () => {
    ...
    const added = await w.apply("hr", "holiday.add", {
      calendar_id: calendarId,
      fields: {
        name: "Moon Day",
        holiday_type: "Public",
    ...
    expect(
      await db.transaction((tx) =>
        hoursOn(tx, w.workspaceId, employeeId, "2026-05-04", allowAll),
      ),
    ).toBe(0);
    const confirmed = await w.apply("hr", "holiday.confirm", {
      holiday_id: holidayId,
      actual_date: "2026-05-05",
    });
    expect(confirmed.status).toBe("applied");
    expect(
      (await db.transaction((tx) => getNode(tx, w.workspaceId, holidayId)))?.record,
    ).toMatchObject({
      date: "2026-05-05",
      estimated_date: "2026-05-04",
      is_provisional: false,
      confirmed_by: w.people.hr!.userId,
    });
    expect(
      await db.transaction((tx) =>
        hoursOn(tx, w.workspaceId, employeeId, "2026-05-04", allowAll),
    ...
    expect(
      await db.transaction((tx) =>
        hoursOn(tx, w.workspaceId, employeeId, "2026-05-05", allowAll),
      ),
    ).toBe(0);
    expect(
      (
    ...
    expect(
      (
        await w.apply("team", "holiday.add", {
          calendar_id: calendarId,
          fields: { name: "Denied", date: "2026-06-01", holiday_type: "Company" },
        })
      ).status,
    ).toBe("rejected");
  });
