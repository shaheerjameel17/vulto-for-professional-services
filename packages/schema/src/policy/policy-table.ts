...
/**
 * Stage 3 (A004-T21). The same lookup as `resolvePermission`, but returning
 * the cell's own outcome and its row-level `scope` instead of the F128
 * ceiling. The server interceptor uses it to decide whether a qualified grant
 * ("own only", "direct reports") can be honored for a specific row. It adds no
 * behavior to `resolvePermission`; `policy-scope.test.ts` asserts that
 * collapsing this result reproduces `resolvePermission` for every cell.
 */
...
export function resolvePolicyCell(
  ...
  nodeType: NodeType,
  partitionKey: string | null,
): PolicyCellResolution {
  if (INHERITED_UNRESOLVABLE_NODE_TYPES.has(nodeType)) {
    return { outcome: "none", scope: "inherited-unresolvable" };
  }
  ...
  if (partitionKey !== null) {
    const exact = MATRIX_OVERRIDES[`${nodeType}:${partitionKey}`]?.[role];
    if (exact) return literal(exact);
  }
  const bare = MATRIX_OVERRIDES[nodeType]?.[role];
  if (bare) return literal(bare);
  ...
  return literal(DEFAULT_CLASS_MAPPING[privacyClass][role]);
}
