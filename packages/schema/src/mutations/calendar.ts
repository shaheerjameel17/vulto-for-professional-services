...
export const workingWeekSchema = z
  ...
  .superRefine((week, context) => {
    ...
    if (new Set(days).size !== 7 || ![1, 2, 3, 4, 5, 6, 7].every((d) => days.includes(d))) {
      context.addIssue({ code: "custom", message: "working_week must contain ISO weekdays 1 through 7 exactly once" });
    }
  });
...
export const partialWorkingWeekSchema = z
  ...
  .superRefine((week, context) => {
    if (new Set(week.map(({ day }) => day)).size !== week.length) {
      context.addIssue({ code: "custom", message: "working_week cannot repeat a weekday" });
    }
  });
...
const week = (workingDays: readonly number[], hoursByDay: Partial<Record<number, number>> = {}): WorkingWeek =>
...
export function initialWorkingWeekFor(jurisdiction: string): {
  ...
  if (jurisdiction === "PK") return { working_week: week([1, 2, 3, 4, 5, 6], { 6: 4 }), standard_daily_hours: 8 };
  if (jurisdiction === "AE" || jurisdiction === "SA") return { working_week: week([7, 1, 2, 3, 4]), standard_daily_hours: 8 };
...
const holidayFields = z.object({
  ...
  if (fields.date === undefined && (!fields.is_provisional || fields.estimated_date === null)) {
    context.addIssue({ code: "custom", path: ["date"], message: "date is required unless a provisional holiday has an estimated_date" });
  }
});
...
export const holidayCancel = defineMutation({
  name: "holiday.cancel",
  input: z.object({ holiday_id: uuidV4Schema, expected_version: z.int().positive() }).strict(),
  tier: 0,
  onlineOnly: false,
  stateTransition: true,
});
...
export const calendarGetInputSchema = z.object({ entity_id: uuidV4Schema, as_of: z.iso.date().optional() }).strict();
export const workingDaysDateInputSchema = z.object({ employee_id: uuidV4Schema, date: z.iso.date() }).strict();
export const workingDaysRangeInputSchema = z.object({ employee_id: uuidV4Schema, from: z.iso.date(), to: z.iso.date() }).strict();
export const workingDaysNextInputSchema = z.object({ employee_id: uuidV4Schema, from: z.iso.date(), n: z.int().positive().optional() }).strict();
export const workingDaysAddInputSchema = z.object({ employee_id: uuidV4Schema, from: z.iso.date(), n: z.int().nonnegative() }).strict();
