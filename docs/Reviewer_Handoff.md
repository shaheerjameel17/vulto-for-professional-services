...
**Last updated:** 22 September 2026, after ruling on F218 (a third Stage 9 attempt blocked before implementation). Stage 7 merged as `dec4eb3`; Stage 8 merged as `5897b7b`, reviewed and ruled. Stage 9 (the real shell bootstrap, redefined from F216) has now been attempted three times and stopped three times, correctly every time: F216 (the shell was never wired to the backend at all — `FDN-69`, never built), then F217 on the corrected brief (F216's ruling assumed `session.activeOrganizationId` was populated somewhere; nothing populates it, online or off), then F218 on the twice-corrected brief (the F217-updated sign-in rule and the offline-cache-boot rule fired on the same event — a cold offline load — and nothing said which won). All three are ruled. `stage-9-reconnect-shell`, `stage-9-shell-bootstrap` and `stage-9-shell-bootstrap-v2` (all pushed) each hold only one blocked attempt's discovery; none has bootstrap or Reconnect code. The next attempt should start clean on a fourth branch name. Two different builders have worked this project now — Claude Code through Stage 8, OpenAI Codex from Stage 9 — the loop is agent-agnostic; see section 1.
...
| Findings (F54–F218, one gap at F198) | `docs/Foundations_Findings.md` (the table is authoritative; the count paragraph is recounted programmatically) |
...
| **9 The real shell bootstrap** | **blocked three times, all ruled — F216 (22 Sept), F217 (22 Sept), F218 (22 Sept); brief updated in place; three abandoned branches (`stage-9-reconnect-shell`, `stage-9-shell-bootstrap`, `stage-9-shell-bootstrap-v2`), none has bootstrap code** | — |
...
## 8. Stage 9 review — F216, then F217, then F218, all ruled
...
**Pattern worth naming.** Three stops in a row on the same stage is not the loop failing — it's exactly what "read the code, not the report" and "no recorded exceptions where a clean fix exists" are for. Every finding was real, every builder stopped instead of inventing a design locally, and every ruling was grounded in code or a pattern that already existed (Better Auth's org plugin; the `revokeWorkspaceAdmission` membership-check pattern; the sync engine's own network-vs-auth failure split) rather than new machinery. F218 in particular came from wording *this review itself* introduced in the F217 fix — a reminder that each fix to a blocked stage needs the same scrutiny as the original brief, not less. If a fourth attempt stops again, treat it the same way: verify before ruling, don't assume the brief is complete just because rounds have passed.

## 9. Stage 9 — the real shell bootstrap (briefed, F216 + F217 + F218)

Full brief in `docs/Claude_Code_Build_Prompt.md` Part 3 (updated in place for F218 — read it directly, don't reconstruct it from this summary or from the Amendments entries). Scope: wire `organizationClient()` into `auth-client.ts`; set `session.activeOrganizationId` server-side in `confirmWorkspaceAdmission` for a person's sole active membership; a sign-in path gated on a *confirmed* no-session answer (`error` null or carrying a numeric `.status`), not merely "no data"; a real `createGraphClient` mount, replacing `Shell.tsx`'s fixture props; offline cold start from a single `session_hint` cache, enumerated without a user filter when no session ever resolved (more than one cached workspace is unresolved, not a silent pick); a `401`/`access-revoked` signal exposed at the shell boundary (not rendered — that's Stage 10). No fixture screen is touched. `FDN-114` (Stage 10), `FDN-115` and `FDN-116` (the two abandoned earlier Stage 9 attempts) all stay as-is; file a new Team FDN issue for this attempt. **Start a fresh branch** — `stage-9-reconnect-shell`, `stage-9-shell-bootstrap` and `stage-9-shell-bootstrap-v2` are all abandoned, holding only their respective findings' discovery.
