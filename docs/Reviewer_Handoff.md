...
**Last updated:** 22 September 2026, after ruling on F217 (a second Stage 9 attempt blocked before implementation). Stage 7 merged as `dec4eb3`; Stage 8 merged as `5897b7b`, reviewed and ruled. Stage 9 (the real shell bootstrap, redefined from F216) has now been attempted twice and stopped twice, correctly both times: first F216 (the shell was never wired to the backend at all — `FDN-69`, never built), then, on the corrected brief, F217 (F216's ruling assumed `session.activeOrganizationId` was populated somewhere; nothing populates it, online or off). Both are ruled. `stage-9-reconnect-shell` and `stage-9-shell-bootstrap` (both pushed) each hold only one blocked attempt's discovery; neither has bootstrap or Reconnect code. The next attempt should start clean on a third branch name. Two different builders have worked this project now — Claude Code through Stage 8, OpenAI Codex from Stage 9 — the loop is agent-agnostic; see section 1.
...
| **9 The real shell bootstrap** | **blocked twice, both ruled — F216 (22 Sept), F217 (22 Sept); brief updated in place; two abandoned branches (`stage-9-reconnect-shell`, `stage-9-shell-bootstrap`), neither has bootstrap code** | — |
...
## 8. Stage 9 review — F216 then F217, both ruled
...
**Pattern worth naming.** Two stops in a row on the same stage is not the loop failing — it's exactly what "read the code, not the report" and "no recorded exceptions where a clean fix exists" are for. Both findings were real, both builders stopped instead of inventing, and both rulings were grounded in code that already existed (Better Auth's org plugin; the existing membership-check pattern from `revokeWorkspaceAdmission`) rather than new machinery. If a third attempt stops again, treat it the same way: verify before ruling, don't assume the brief is now complete just because two rounds have passed.

## 9. Stage 9 — the real shell bootstrap (briefed, F216 + F217)

Full brief in `docs/Claude_Code_Build_Prompt.md` Part 3 (updated in place for F217 — read it directly, don't reconstruct it from this summary or from the Amendments entries). Scope: wire `organizationClient()` into `auth-client.ts`; set `session.activeOrganizationId` server-side in `confirmWorkspaceAdmission` for a person's sole active membership; a Better-Auth-session-but-no-workspace-call sign-in path; a real `createGraphClient` mount, replacing `Shell.tsx`'s fixture props; offline cold start from a single `session_hint` cache (more than one cached workspace is unresolved, not a silent pick); a `401`/`access-revoked` signal exposed at the shell boundary (not rendered — that's Stage 10). No fixture screen is touched. `FDN-114` (Stage 10) and `FDN-115` (the abandoned first Stage 9 attempt) both stay as-is; file a new Team FDN issue for this attempt. **Start a fresh branch** — both `stage-9-reconnect-shell` and `stage-9-shell-bootstrap` are abandoned, holding only their respective findings' discovery.

**What to check in the Stage 9 report:** that `organizationClient()` and the `confirmWorkspaceAdmission` change together are genuinely what closed both gaps; that a second-workspace creation/join by someone already active elsewhere does not switch them (test, not assertion); that no screen outside the shell's own bootstrap changed; that offline boot was tested for both the single-cache and multi-cache cases, not just reasoned about; that the refusal-signal shape is documented well enough for Stage 10 to consume without re-deriving it; and that "no session" truly never triggers a workspace call.
