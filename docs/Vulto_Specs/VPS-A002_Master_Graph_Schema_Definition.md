...
**`created_by`, `updated_by` and `soft_deleted_by` may name a system principal's own reserved identity, never a fabricated or blank one — F272, 24 September 2026.** A named system principal (`packages/schema/src/policy/principal-policy.ts`'s `SYSTEM_PRINCIPAL_NAMES`) sometimes writes a node or edge with no member present at all — the first case is `VRS-F010`'s post-submit anomaly evaluator. `user_id UUID` above is satisfied by a dedicated `User` node reserved for that principal, one per principal per workspace: universal fields only, no linked Employee, no `WorkspaceMembership` edge, so it is never admitted through the normal signup/invitation path and never surfaces in any people- or membership-facing listing, which all reach `User` through `WorkspaceMembership` today. It is provisioned lazily and deterministically — derived from `(workspace_id, system_principal_name)`, created (`INSERT ... ON CONFLICT DO NOTHING`, race-safe) the first time that principal writes anything in a given workspace, never by a founding-time backfill, since `SYSTEM_PRINCIPAL_NAMES` grows as later stages add jobs. Its own `created_by`/`updated_by` are self-referential, the same bootstrap resolution `services/api/src/graph/founding.ts` already uses for the founder's own `User` row. This was chosen over two rejected alternatives: attributing the write to the acting member (false, and incoherent for `VRS-F010` specifically, where that member is deliberately denied read access to the very record they'd be recorded as authoring) and a fabricated UUID backed by no row at all (passes the format check with nothing to resolve, an unrecoverable dead end for any future feature that reads `created_by` back). A closed, exhaustive `SYSTEM_PRINCIPAL_DISPLAY_NAMES` table (mirroring `SYSTEM_PRINCIPAL_NAMES`'s own closed shape) pairs each system principal with a plain, human, non-technical label — never the internal principal name — for whichever future stage displays "created by" in the interface; `timesheet-anomaly-evaluate`'s is "Automatic Review." No node type this convention already excludes is affected, and no existing member-authored record changes shape.
...
An edge's `created_by` follows the same convention as a node's (above): a system principal's own reserved `User` identity, never a fabricated or blank one (F272).
...
| `promoted_to` | Employee (to) | `operational` | Promotion changes only Employee's Tier 0 operational identity fields (name, email, employment type, start date, employee code); it never touches compensation (F254, extending F208/F235's own precedent) |
...
**A system principal writing a graph node or edge needed its own real identity for `created_by`/`updated_by` — F272, 24 September 2026.** Every universal field above required a genuine `user_id UUID`; nothing in the convention anticipated a write with no member behind it at all, and `VRS-F010`'s post-submit anomaly evaluator (F270/F271) is the first feature to need one. Resolved with a reserved, non-membership `User` node per system principal per workspace, lazily and deterministically provisioned, self-attributed, paired with a closed table of plain human labels for later interface use — never the acting member's identity, never a fabricated UUID with nothing behind it. See Universal node/edge conventions above for the corrected text.
...
**A Tree-derived edge's interval could not be keyed to causal ordering.** F104's correction said the materialized edge's intervals were keyed to the Tree operation's causal ordering. Implementing it showed that sentence asks one mechanism to answer two unrelated questions: which concurrent move wins, and when the winning move took effect. The first is causal and has a correct answer — `(lamport, peer)`, since Lamport alone ties between concurrent moves. The second is a real-world date, and no deterministic one exists anywhere in a Loro Tree operation: the op's own timestamp is zero unless explicitly recorded and is the originating device's wall clock at one-second granularity when it is. Writing a causal key into `effective_from` would have dated every reporting line to 1970 and broken the temporal queries these columns were promoted to first-class fields to serve. The effective date is now carried on the move operation as replicated Tree data. Recorded as F124, found by implementation rather than by reading.

**`managed_by`'s two representations had no stated authority.** [[VPS-A001_Technology_Stack_and_Engineering_Foundations|VPS-A001]] requires the reporting hierarchy on Loro's Movable Tree for cycle-free concurrent merge; this document separately requires every single-active-outgoing relationship, `managed_by` included, as a temporal edge with preserved history. Persisting both without a rule connecting them left two answers to "who manages this employee." The Tree is now stated as sole write target and sole authority on the current answer; the edge is a one-way materialization of the Tree's resolved state, keyed to the Tree operation's causal ordering so every device converges on the same edge history regardless of merge order. Recorded as F104, closed by founder ruling; FDN-50 proves the concurrent-same-employee case in its own verification, with a real Loro merge rather than a simulated one.
