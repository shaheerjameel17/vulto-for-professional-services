...
- **Organization plugin routes stay closed, reads included (F219).** Every `/api/auth/organization/*` route is refused with 404, not only its mutations — a second write path beside `workspace.create`/`confirmWorkspaceAdmission`/`revokeWorkspaceAdmission`/`changeWorkspaceRole` is never opened, and a second read path with its own filtering semantics is not opened either. Two narrow caller-facing exceptions exist instead, both session-gated with no workspace scope (the same shape as `/devices/retire`), both applying the exact predicate `requireCurrentWorkspaceSession` and `confirmWorkspaceAdmission` already use: `POST /workspace/list-active-memberships` returns the caller's `{ workspaceId, workspaceName }` for memberships that are `active`/`confirmed` on both the membership and its organization — nothing about pending or revoked memberships, no role or membership-id data. `POST /workspace/activate-sole-membership` (F220) re-derives that same set itself, trusting no client-supplied workspace id, and — only when exactly one such membership exists — sets `session.activeOrganizationId` for every session row that user holds, the same write `confirmWorkspaceAdmission` performs for the confirming session at admission time. It exists because that write only ever fires once, at confirmation, and cannot reach a session created by a later sign-in; it only ever writes from `null` and never overwrites an already-set active organization, so it cannot become a hidden workspace switcher.
