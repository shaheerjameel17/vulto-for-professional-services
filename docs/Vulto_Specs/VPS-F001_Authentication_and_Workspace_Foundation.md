...
- **Organization plugin routes stay closed, reads included (F219).** Every `/api/auth/organization/*` route is refused with 404, not only its mutations — a second write path beside `workspace.create`/`confirmWorkspaceAdmission`/`revokeWorkspaceAdmission`/`changeWorkspaceRole` is never opened, and a second read path with its own filtering semantics is not opened either. The one caller-facing exception is `POST /workspace/list-active-memberships`, a session-gated route with no workspace scope (the same shape as `/devices/retire`) that returns only the caller's `{ workspaceId, workspaceName }` for memberships that are `active`/`confirmed` on both the membership and its organization — the same predicate `requireCurrentWorkspaceSession` and `confirmWorkspaceAdmission` already apply. It exists solely so a session with no active organization yet can resolve one; it returns nothing about pending or revoked memberships and carries no role or membership-id data.
