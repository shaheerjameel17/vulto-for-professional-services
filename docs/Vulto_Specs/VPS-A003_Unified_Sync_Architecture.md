...
### Audit journal transport and retention

AuditEntry also has a deliberately separate storage convention from the ordinary mutable workspace document. The permission interceptor appends first to a sealed local audit journal and durable outbox. An authenticated idempotent append then replicates the exact event into a workspace-isolated Tier 2 server journal. This is an audit-specific exception to ordinary graph-delta transport: the server is permitted to read this closed metadata, retain it for the workspace lifetime and serve aged-out pages on demand, but it does not become the canonical customer graph and cannot invent, edit or delete an event.

The device materializes only the local audit retention window. Removing an old AuditEntry from that projection is cache eviction, never a replicated deletion; the retained server journal supplies older pages through [[VPS-F004_Silent_Audit_Log|VPS-F004]]'s permission-aware query. An event is keyed by its globally unique `audit_entry_id`: replaying identical content is idempotent and presenting different content under that identifier is rejected.

For Tier 1 and Tier 3 access, a successful local journal append is part of the release boundary. Protected data is not returned until its event is locally durable. Connectivity is not part of that boundary: after the local append, an offline operation completes and the outbox retries. If local audit durability fails, protected data remains withheld and the runtime fails closed rather than continuing an unaudited session.

This convention is immutable and tamper-resistant across every supported product and API path, including paths available to Owner. It does not claim cryptographic proof that a deliberately modified client on an already-authorized device reported every local read. That stronger hostile-client property requires its own future architecture decision and is outside FDN-68.
...
| A003-T09 | Every sync delta MUST be logged to [[VPS-F004_Silent_Audit_Log|VPS-F004]], including its tier. The relay's observability surface is proven opaque by FDN-51 Stage 5 against the surface that exists at that time — today a single `tracing` event stream, no metrics or tracing exporter. Introducing any metrics sink, tracing exporter, or new telemetry surface on the relay MUST re-run Stage 5's opacity scan against it before it ships; the opacity claim does not extend to a surface Stage 5 never examined |
| A003-T10 | The sync engine's wire format and codec MUST have one authoritative definition: the Rust `[lib]` in `services/sync-engine/src/wire/`, which opens no socket and depends on no runtime and drives the native `[[bin]]` relay and any future mobile host directly. The browser client MAY use a TypeScript implementation of the identical format (`packages/graph/src/sync/wire.ts`) kept byte-for-byte equivalent by A003-T42's versioned shared vectors, rather than the `[lib]` compiled to WASM; the two implementations MUST NOT diverge. Consolidating onto one compiled artifact used unchanged on every platform is a permitted future step, not a current requirement (FDN-51 Stage 6, F188) |
...
**Authorization is re-checked for the life of the connection, not only at `Hello`.** A device revoked, a session expired or a membership removed while a socket is open is a revocation event the relay must act on — a relay that only gates new connections leaves a compromised device syncing until it happens to disconnect. The relay re-runs the full admission check on a fixed interval and closes any connection that no longer passes with a typed `Error` (kind `unauthenticated`). The interval bounds worst-case exposure; it does not need to be instantaneous, because central revocation (A003-T07) and device wipe (A003-T16) remain the primary controls and the relay holds no plaintext regardless.

| ID | Specification |
|---|---|
| A003-T36 | The `services/sync-engine` `[[bin]]` relay MUST use WebSocket over TLS 1.3 minimum as its device transport, on the Tokio async runtime with the Axum framework and Axum's built-in WebSocket upgrade. HTTP/2 streaming and long-polling are rejected. The `[lib]` shared core MUST remain transport-agnostic — it produces and consumes framed message bytes only and MUST NOT open a socket or depend on a runtime. **TLS 1.3 MUST be terminated at least at the ingress in production.** In-process `wss://` on the relay itself is an optional capability, gated on a `SYNC_TLS_CERT_PATH` / `SYNC_TLS_KEY_PATH` pair mirroring `services/api`'s `API_TLS_CERT_PATH` / `API_TLS_KEY_PATH`; it is deliberately not implemented in FDN-51 Stage 2 (the local relay runs plain `ws://`, exactly as `services/api` runs plain HTTP locally without a cert path) |
...
| A003-T45 | The relay MUST re-evaluate a connection's authorization for the lifetime of the socket, not only at `Hello`. It MUST re-run the full admission check (the session, membership and device-revocation conditions of A003-T38's authorization) on a bounded fixed interval and MUST close a connection that no longer passes with a typed `Error` (kind `unauthenticated`). A revoked device, expired session or removed membership MUST be disconnected within one interval. The interval is a deployment parameter, defaulting to ten seconds; a client MUST treat this close as it treats any other transport drop — reconnect, and be denied at `Hello` if the revocation is real |
| A003-T46 | AuditEntry MUST use [[VPS-F004_Silent_Audit_Log|VPS-F004]]'s dedicated storage convention: a sealed local append journal and durable outbox, an authenticated idempotent append into a workspace-isolated Tier 2 server journal, and a local retention-window projection. It MUST NOT use the ordinary mutable node-fragment write path |
| A003-T47 | The audit server journal MUST retain each accepted event for the workspace lifetime, accept an identical `audit_entry_id` replay idempotently, reject different content under an existing identifier, and expose no generic update or delete path. It is a durable retained replica and historical-query source, not the canonical customer graph |
| A003-T48 | Evicting an AuditEntry from the local retention-window projection MUST NOT emit a deletion or remove the retained event. An authorized historical query MUST fetch aged-out pages on demand through opaque workspace-bound cursors |
| A003-T49 | A Tier 1 or Tier 3 result MUST NOT cross the Worker boundary until its audit event is durable in the sealed local journal. Server unavailability after that append MUST NOT block offline work; the durable outbox retries. Local audit-persistence failure MUST withhold the protected result and fail the runtime closed before another operation |
| A003-T50 | FDN-68's tamper-resistance claim covers every supported product and API path, including Owner paths. Cryptographic proof against a deliberately modified client on an already-authorized device is explicitly out of scope and MUST NOT be implied without a separate future architecture decision |
...
**Server opacity is proven adversarially, and the claim is scoped to the surface that exists — FDN-51 Stage 5.** The A003-T43 requirement — no relay code path, PostgreSQL column, log line, metric or telemetry event carries Tier 1/3 plaintext or a usable key — is closed by an adversarial real-stack proof (`services/sync-engine/tests/relay_pg.rs`). Real FDN-52 Tier 1 and Tier 3 protected envelopes, generated by the real constructions and checked in as `tests/vectors/opacity/{tier1,tier3}.json`, are pushed through the relay; the proof then scans every `sync_*` column of every row and the relay's entire `TRACE`-level log for every secret in the Tier 1/3 key hierarchy — protected plaintext, per-document key, Tier 3 root key, Tier 1 recovery secret and shares, reader-set unwrap material, Tier 1 identity private key and the `Tier1IdentityTransfer` (A003-T32) it carries, Tier 3 recovery code, the WebAuthn PRF *result* (never its non-secret *input*), the pre-hash sync ticket, and the session token — in every plausible representation, and finds none. The founder ruled two conditions on this proof: it is delivered as committed fixtures pushed by the Rust integration test rather than a browser path (the server cannot distinguish origin and never opens a payload, so a browser test would re-prove client crypto, not server opacity); and, because the proof rests partly on the relay having only a single `tracing` event stream and no metrics or tracing exporter today, A003-T09 now records that any future exporter or telemetry surface must re-run this scan against itself before it ships.

**Authorization is re-checked for the life of the connection — FDN-51 Stage 3.** Implementing the relay showed the Stage 0 contract gated only new connections: a device revoked while its socket stayed open kept syncing until it disconnected on its own. A003-T45 closes that hole — the relay re-runs the full admission check on a bounded interval (ten seconds by default) and evicts a connection that no longer passes. The founder ruled the interval periodic rather than event-driven: a notification channel from the auth service to the relay is a larger mechanism than the exposure justifies, given that central revocation and device wipe remain the primary controls and the relay holds no plaintext. Stage 3 also replaced the in-memory live fan-out with a per-workspace doorbell over the durable delta log — the log's `ORDER BY cursor` is the single ordering authority, so a wake that races ahead of a lower cursor can no longer drop a delta for a live consumer — and made reconnect replay page through a backlog larger than one batch. Auto-replay from the client's last acknowledged cursor stays the only resume contract; no `Hello` opt-out and no protocol-version bump were added.
