...
**Every row belongs to exactly one workspace — F204.** `graph_nodes` is keyed by `(workspace_id, node_id)` and `graph_edges` references its endpoints by that pair, so an edge can never join two workspaces. The Workspace node's `workspace_id` is its own `node_id`. A `User`, which [[VPS-A002_Master_Graph_Schema_Definition|VPS-A002]] scopes to no single workspace, is stored as one row per workspace it belongs to, all under its Better Auth account id, each written only by the membership projection. Every other node ID remains globally unique, enforced by a partial unique index. One identity, the Better Auth account, stays global; everything in the graph is tenant-scoped, which is what per-workspace residency, export and erasure under [[VPS-A008_Trust_and_Data_Protection_Program|VPS-A008]] require.
...
**Every device subscribes to exactly two fixed shapes**, through the API's authenticated shape proxy: `graph_nodes` where `workspace_id` is the session's workspace and `node_id` is in the person's `sync_node_audience` rows, and the same for `graph_edges`. The proxy sets the table, the where clause and its parameters from the verified session; a client supplies none of them. Electric tracks the subquery, and reports a row entering the audience as an ordinary insert and a row leaving it as a `move-out` event, not a delete: every row it delivers carries tags (one per audience entry that admits it), and a move-out names the tags that no longer apply. The device cache therefore records each row's tags, removes the tags a move-out names, and deletes any row left with none, in the same transaction as the replication cursor; a client that applied only inserts, updates and deletes would keep rows the person may no longer read. One source — the interceptor — decides both what a person may query and what their device holds, and a conformance test (A003-T57) asserts, for every role and privacy class, that every audience row is one the interceptor permits.
...
| A003-T72 | Devices MUST reach Electric only through the API's authenticated shape proxy, which MUST set table, where clause and parameters server-side from the verified session and MUST reject any client-supplied table, where or column parameter. The only query parameters it forwards are Electric's protocol parameters — `offset`, `handle`, `live`, `cursor`, `expired_handle` and `cache-buster` — and `log`, only with the value `full`; every other parameter is refused. Exactly two shape templates exist — nodes and edges filtered by the sync audience — and no other shape MAY be served |
