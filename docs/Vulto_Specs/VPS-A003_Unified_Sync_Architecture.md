...
**F172–F175 make continuity, atomicity and the browser boundary explicit.** Tier 1 identity belongs to the user/workspace and survives ordinary Worker recreation through a sealed wrapped record; existing-device transfer has a fixed authenticated construction independent of SLIP-0039. FDN-84's sealed compare-and-swap is the protected-state linearization point, while whole-store historical rollback remains an honest future limitation. The production Worker protocol cannot generically open internal sealed records, and browser-required ceremonies use a narrow trusted Window path without claiming resistance to active same-origin compromise.
