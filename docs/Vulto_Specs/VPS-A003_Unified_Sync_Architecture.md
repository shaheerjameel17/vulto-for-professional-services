...
```
                       ┌───────────────────────────────┐
  Device (browser/app) │  SQLite cache  (Tier 0 only)  │◄── replication (Sync Streams)
                       │  Upload queue  (Tier 0 writes)│──► graph.applyMutations (tRPC)
         ...
         PostgreSQL — canonical graph, protected field store, audit journal
                                        │  logical replication
                                        ▼
         PowerSync Service — evaluates Sync Streams, streams each device its slice
```
...
**The replication publication is an allowlist.** Only `graph_nodes` and `graph_edges` are published to the replication service, and a CI gate (A003-T58) fails the build if any other table, or any column holding protected content, becomes reachable from a Sync Stream.
...
**Revocation is immediate at the server** and needs no key rotation, because no device holds a key. When a person loses access — offboarding, a role change, becoming the subject of a record — the interceptor denies their next request, their Sync Streams narrow, and the replication service instructs their devices to remove rows they may no longer hold. An explicitly revoked device is told to erase its cache entirely on its next connection.
...
| A003-T57 | Sync Streams MUST be generated from [[VPS-A004_Graph_Permission_Layer|VPS-A004]]'s policy table. A conformance suite MUST assert, for every role and privacy class, that the rows each stream delivers are a subset of what the interceptor permits for that person |
| A003-T58 | The replication publication MUST contain only `graph_nodes` and `graph_edges`. CI MUST fail if any other table, or any Tier 1 or Tier 2 value, is reachable from a Sync Stream |
...
**GIVEN** a Sync Stream definition is changed so that a Standard stream would include a Tier 2 node type
**WHEN** CI runs
**THEN** the stream conformance gate fails the build
...
**Replication uses PowerSync; the API remains ours.** A Linear-grade feel needs a local cache, optimistic writes and pushed updates. Building a bespoke replication protocol would recreate the cost this reversal removes. PowerSync was selected over Zero and ElectricSQL because it is the one that supports queued offline writes to our own API and replicates PostgreSQL into client SQLite, which lets `packages/graph`'s existing SQLite query layer carry over. Zero rejects writes while disconnected, which contradicts the offline requirement above; ElectricSQL leaves the write path and local store to the application. Revisit if PowerSync's licensing or maintenance changes materially.
...
- [[VPS-A004_Graph_Permission_Layer|VPS-A004]] — the interceptor and the policy table Sync Streams are generated from
