...
- **TimesheetEntry's row-scoped grant required a new resolution branch reading its own stored field; TimesheetAnomalyFlag's required no new mechanism at all.** The permission interceptor's row-scope check only resolved a row's subject Employee for the Employee and BurnoutAlert node types; every other type fell to its conservative default and refused the grant outright. `employee_id`'s own direct-field placement on TimesheetEntry (G04 — a Pitch-categorized entry has no Assignment to traverse) is what the new branch reads, via the interceptor's own `getNode`. TimesheetAnomalyFlag is `Manager-restricted`, Tier 2: its fields are not in the plain node record `getNode` reads at all (Tier 1/2 content lives encrypted, separately), so a field read would not have worked — but TimesheetAnomalyFlag already carries a registered `triggered_by` edge to Employee, the same edge BurnoutAlert (also Tier 2) already uses, so the interceptor's existing BurnoutAlert branch reaches it unchanged. Checked directly against the code and closed before this feature's own code was written (F268); Codex correctly caught the Tier 2 field-read defect mid-build and the TimesheetAnomalyFlag mechanism was corrected before merge (F269).
...
- **A Team Member's own "Full (own only)" grant could not authorize the very first TimesheetEntry or TimesheetWeekSubmission they ever create, because the pipeline authorizes a write before it happens and "own" is normally judged by reading the row's stored `employee_id` — on a create, no such row exists yet.** Every node type ever created before this feature was created only by a role holding unscoped Full, so this gap had never been exercised; it is the create-time half of the "self-service pattern" [[VPS-A004_Graph_Permission_Layer|VPS-A004]] already names for six node types this feature is the first to build. Founder-ruled, platform-wide, not specific to this feature: a mutation's own already-validated args declare the new row's subject (`WriteChange.declaredSubjectEmployeeId`, distinct from the reader-set gate's own `subjectEmployeeId`), compared against the caller's own identity exactly as an existing row's stored field already is; the pipeline then independently re-reads the row it just wrote and refuses the whole transaction if it does not match what was authorized, so a future mutation's declared subject and its actual write can never silently drift apart. See [[VPS-A004_Graph_Permission_Layer|VPS-A004]]'s self-service pattern and its F274 decision entry for the full mechanism (F274).
- **`logged_against` (TimesheetEntry to Assignment or Pitch) denied both its own endpoints for an ordinary Team Member, independently of F274's fix, because its registration declared no `readSufficientEndpoints` for either.** Split into two halves. The Assignment half traced directly to `assigned_to`'s own registered precedent (F262) — the identical shape of problem, a writer holding only Read on an endpoint that should still count as sufficient — no founder consultation needed: `logged_against` gains `readSufficientEndpoints: { Assignment: true }`, letting a Team Member's existing `READ_ANY()` on Assignment count as sufficient without widening anyone's Assignment visibility. The Pitch half is a genuine fork: Team Member's outcome on `Pitch:identifying` is unconditionally `NONE`, a deliberate F264–F267 restriction, not an oversight, so no `readSufficientEndpoints` declaration could touch it. Founder-ruled: a new, reusable `PARTICIPANT_GRANTS` registry and `participantGrantSatisfied` resolver, wired only into `edgeRoleDecision`, grant a Team Member currently staffed on a Pitch (via a currently-active `staffed_on` edge) read-sufficient access to write `logged_against` against it, while an unstaffed Team Member remains refused. See [[VPS-A004_Graph_Permission_Layer|VPS-A004]]'s participant-scoped grants mechanism and its F275 decision entry for the full mechanism (F275).
