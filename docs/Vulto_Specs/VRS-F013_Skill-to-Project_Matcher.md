...
**`certified` reads `false` everywhere, for every match, until [[VRS-F041_Certification_and_Training_Tracker|VRS-F041]] exists (F296).** This feature's own API contract has carried a `certified` field since it was written, but nothing in the graph can produce anything else today: `has_skill`'s edge metadata has no reserved slot for it, `holds_certification` is registered but unwritable, and `VRS-F041`'s own design resolves a certification's effect as an upgrade to `verified`, not a second, independent flag. This is an accepted forward dependency, the same shape as [[VPS-F002_Local-First_Search|VPS-F002]]'s own deferred `Cmd+K` trigger — not a defect in this stage's build.
