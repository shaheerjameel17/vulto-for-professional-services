...
**Do:**
1. **The calendar/pattern mutation module.** New `packages/schema/src/mutations/calendar.ts`, mirroring `entity.ts`'s exact pattern (checks/validate/apply, `ServerMutation<MutationArgs<...>>`): argument schemas for `calendar.update`, `holiday.add`, `holiday.confirm`, `holiday.cancel`, `pattern.set`, `pattern.clear`. Register all six in the shared `MUTATIONS` registry (`packages/schema/src/mutations/index.ts`) and `services/api/src/mutations/pipeline.ts`'s `IMPLEMENTATIONS`.
2. **`calendar.get`/`calendar.update`.** `calendar.get(entityId, asOf?)` reads the Entity's Active `WorkingCalendar`, or, with `asOf`, the version active at that date by following the `supersedes` chain — a small dedicated resolver in `services/api`, the same placement `entity.resolveForEmployee` established in Stage 11. `calendar.update(calendarId, workingWeek, dailyHours)` never edits in place (G01): it writes a new `WorkingCalendar` with `lifecycle_status: "Active"`, connects it from the prior version via the existing `supersedes` edge, and marks the prior version `lifecycle_status: "Superseded"` — both writes committed together. Validate `working_week` as exactly seven entries keyed 1–7, each with a boolean `is_working` and a non-negative decimal `hours`.
3. **Holiday mutations.** `holiday.add(calendarId, fields)` creates a `Holiday` scoped to the calendar; when `is_provisional` is true, `date` defaults to `estimated_date`. `holiday.confirm(holidayId, actualDate)` sets `date` to `actualDate` and stamps `confirmed_at`/`confirmed_by`, leaving `estimated_date` untouched as the record of the shift — no `VPS-F004` `AuditEntry` call anywhere in this path (F225). `holiday.cancel(holidayId)` sets `lifecycle_status: "Canceled"`. None of the three resolve `applies_to_locations` themselves — that belongs to `workingDays.*`'s read-time resolution order (item 5), not to these writes.
4. **`pattern.set`/`pattern.clear`.** Single-active-with-history for a versioned node, the same shape `employee.setEntity` used for an edge in Stage 11: `pattern.set(employeeId, workingWeek, effectiveFrom)` closes the employee's current Active `WorkingPattern` (`effective_to` set, `lifecycle_status: "Superseded"`) and creates a new Active one; reject at write time (G04) if the new `effective_from` falls inside an existing pattern's still-open range for the same employee. `pattern.clear(employeeId, effectiveFrom)` closes the current pattern with no replacement. A weekday omitted from `workingWeek` is stored as omitted, never defaulted to a zero-hours row (G03) — item 5's read-time resolution performs the inheritance, not this write.
...
- Every Entity has an Active `WorkingCalendar` immediately on creation, whether Stage 11's founding-bootstrap default Entity or one created later via `entity.create` — proven by a test, not asserted (G02).
...
- Two overlapping `WorkingPattern` effective ranges for the same employee are rejected at write time (G04).
...
- The resolution order is proven end to end against the spec's own acceptance criteria: the AE Friday exclusion, the PK 5.5-day/44-hour week, the UK→March pattern-effective-date split, and the UAE Ramadan 0.75-factor example.
- `workingDays.*` is the only code path anywhere in the diff that reads `WorkingCalendar`, `Holiday` or `WorkingPattern` to decide whether a day is a working day — grep the diff to confirm nothing else does (G08).
...
**What to check in the Stage 12 report:** that `calendar.update`/`pattern.set`/`pattern.clear` all supersede rather than edit in place (G01, G04); that a newly created Entity gets an Active `WorkingCalendar` with no manual step (G02); that an omitted `WorkingPattern` weekday inherits at read time rather than being defaulted at write time (G03); that G05's location scoping is tested against employees in two different locations, not asserted; that `holiday.confirm` writes no `VPS-F004` `AuditEntry` anywhere in the diff (F225) — grep for it; that no materialized index, cache table or precomputed row appears anywhere (F226) — grep the diff for anything resembling one; that `workingDays.*` is the sole resolution-order code path (G08); that no file under `apps/` appears in the diff; and that `holiday.importForJurisdiction` is not implemented.
...
Stages 1–11 are merged (Stage 11 as `96d53eb`). Stage 12 — Working Calendar and Working Patterns (`VRS-F004`) — is briefed above (Part 3), server-side only, with F225 and F226 both ruled the same day they were found. `VRS-001`'s Feature Register confirms it as the correct next MVP-order feature after `VRS-F003`. Build it on `codex/stage-12-working-calendar`; `FDN-119` is already created in Linear (Backlog) — move it In Progress. When its report is written and Linear is updated, stop and wait for the founder, same as every stage boundary.
