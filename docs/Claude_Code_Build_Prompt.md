...
- **22 September 2026 — F216 ruled; Stage 9 redefined, original Reconnect brief renumbered Stage 10.** The Stage 9 (Reconnect) brief went to OpenAI Codex this round, a fresh session against the same repo, not Claude Code — the loop is agent-agnostic (see `Reviewer_Handoff.md` section 1). Codex traced the brief before writing UI and stopped with F216 rather than inventing a bootstrap design locally, which is the correct call under this project's decision policy. Verified directly: `apps/roster-web/src/app/(shell)/layout.tsx` wraps only the fixture `Shell.tsx`, which hardcodes `workspaceName="Northgate Studio"` and `syncStatus="Synced"` as literals; `createGraphClient` is mounted nowhere but `/sync-harness`; `authClient.useSession()` appears only on `/auth-ready` and `/devices`, outside the shell, and `/auth-ready`'s own copy says workspace access stays closed until the server confirms the membership projection; `SyncEngine.#handleRevoked` stops every source and publishes `signedOut: true` before any retry could reuse that worker. Every screen under `(shell)/` renders from fixtures (`EMPLOYEES`, a fixed viewer, a client-side `canSeeCompensation()` helper unrelated to the real interceptor). This is `FDN-69`'s scope — the shared shell and workspace bootstrap — never built, sitting in Backlog since before Stage 1; Reconnect assumed it already existed.
  ...
  - **Stop after that.** Wait for "Stage 9 go" (the real shell bootstrap) before starting either Stage 9 or Stage 10.
...
**Linear:** file a new focused issue under Team FDN (same pattern as `FDN-103`/`FDN-104`/`FDN-114`), titled around "the real shell bootstrap (F216)", linked to `FDN-69`'s milestone and citing F216. Set it **In Progress** when you start. `FDN-114` (the old Stage 9 Reconnect issue) stays as-is; it becomes Stage 10's issue once this one is done.

**The ruled contract (F216).** Checked directly against the code before writing this: `services/api/src/auth/config.ts` already configures Better Auth's `organization()` plugin server-side, and the session already carries `activeOrganizationId` — `services/api/src/auth/workspace-session.ts` already clears it to `null` on revocation, and `requireCurrentWorkspaceSession` already treats it as the workspace claim. The gap is entirely client-side: `apps/roster-web/src/lib/auth-client.ts` has no `organizationClient()` plugin, so nothing in the shell can read it.

1. **No session → sign-in.** This needs no workspace call at all. `authClient.useSession()` (already proven working on `/auth-ready`) resolving to no data, once `isPending` clears, is unconditionally "sign in." Do this first and do it purely client-side.
2. **A session → resolve the workspace and mount the real client.** Add `organizationClient()` to `auth-client.ts`'s plugins. Read `user.id` and `session.activeOrganizationId` from the resolved session. Construct the real graph client (`createGraphClient`, the same one `/sync-harness` already exercises) with those two values, sending `x-vulto-workspace-id` per the existing convention (`packages/schema/src/mutations/define.ts`'s `WORKSPACE_HEADER`). Mount it in the shell layout, replacing `Shell.tsx`'s hardcoded `workspaceName` and `syncStatus` props with the client's real state (a real workspace-name read and the client's actual `SyncState`, not a new fixture).
3. **Offline boot stays offline.** `VPS-A003`: "booting offline is restored." If there is no network but a cached `session_hint` (user + workspace identifiers) exists from a prior session, mount the client from the cache without waiting on `authClient.useSession()` to resolve online. Do not make offline boot depend on a network round-trip it didn't need before this stage.
...
5. **Retry, for Stage 10, means re-running this bootstrap.** `#handleRevoked` stops every source; there is nothing left inside a stopped client to retry. Design the bootstrap so it can be re-invoked from scratch (construct a fresh client, same two-step resolution above) — that re-invocation *is* what Stage 10's Retry button calls. Say this explicitly in the report so Stage 10 doesn't have to rediscover it.
6. **Do not touch `Offline`.** It already exists from Stage 6 and is out of scope here.

**Done criteria:**
- A real, authenticated user with a workspace sees the shell mount against the live graph client, not fixtures — proven with a browser test, not asserted from the diff.
- A user with no Better Auth session is sent to sign-in without any workspace call being attempted.
- A cold offline boot with a cached `session_hint` mounts from cache with no network call.
...
**Gates:** the standard four (`pnpm install --frozen-lockfile`, `pnpm stack:up`, `pnpm verify`, `pnpm verify:full`), plus a real browser test proving: (a) an authenticated session with a workspace mounts the real client; (b) no session redirects to sign-in with zero workspace calls; (c) offline cold start with a cached `session_hint` mounts without network; (d) a `401`/`access-revoked` from the client is observable at the shell boundary.

**What to check in the Stage 9 report:** that `organizationClient()` was the actual missing piece, or an explanation of what else was needed if it wasn't enough; that no screen outside the shell's own bootstrap was touched; that the refusal-signal shape is documented clearly enough that Stage 10 can consume it without re-deriving anything; and that offline boot was tested, not just reasoned about.
...
Stages 1–8 are merged. Begin **Stage 9** on the founder's "Stage 9 go", building the real shell bootstrap exactly as F216 ruled it. When its report is written and Linear is updated, stop and wait for the founder — Stage 10 (Reconnect) follows once Stage 9 is approved and merged.
