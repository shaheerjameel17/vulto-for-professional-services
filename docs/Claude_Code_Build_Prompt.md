...
**Do:**
1. **The calendar/pattern mutation module.** New `packages/schema/src/mutations/calendar.ts`, mirroring `entity.ts`'s exact pattern (checks/validate/apply, `ServerMutation<MutationArgs<...>>`): argument schemas for `calendar.update`, `holiday.add`, `holiday.confirm`, `holiday.cancel`, `pattern.set`, `pattern.clear`. Register all six in the shared `MUTATIONS` registry (`packages/schema/src/mutations/index.ts`) and `services/api/src/mutations/pipeline.ts`'s `IMPLEMENTATIONS`. Add a shared `createInitialCalendar(tx, entityId, jurisdiction)` helper in the same module: PK/UK/US/AE/SA get their named template; every other jurisdiction (including `Global`, `IN`, `SG`) gets Monday-Friday, `standard_daily_hours: 8` (F227). Both `entity.create` and `services/api/src/graph/founding.ts`'s `writeFoundingRecords` call this one helper, writing the calendar atomically with the Entity, never as a follow-up.
2. **`calendar.get`/`calendar.update`.** `calendar.get(entityId, asOf?)` reads the Entity's Active `WorkingCalendar`, or, with `asOf`, the version active at that date by following the `supersedes` chain — a small dedicated resolver in `services/api`, the same placement `entity.resolveForEmployee` established in Stage 11. `calendar.update(calendarId, workingWeek, dailyHours, expectedVersion)` checks `expectedVersion` against the calendar being superseded first (`stale-state` on a mismatch, F228), then never edits in place (G01): it writes a new `WorkingCalendar` with `lifecycle_status: "Active"`, connects it from the prior version via the existing `supersedes` edge, and marks the prior version `lifecycle_status: "Superseded"` — both writes, plus copying every still-Active Holiday of the prior version onto the new one (fresh `holiday_in` edge, never a Canceled holiday, F230), committed together. Validate `working_week` as exactly seven entries keyed 1–7, each with a boolean `is_working` and a non-negative decimal `hours`.
...
- The resolution order is proven end to end against the spec's own acceptance criteria: the AE Friday exclusion, the PK 5.5-day/44-hour week, the UK→March pattern-effective-date split, and the UAE Ramadan 0.75-factor example — the PayRun and LeaveRequest examples are out of scope this stage (F231).
...
- `pnpm verify` passes typecheck with real optimistic handlers for all six new mutations — no stub, no-op, or placeholder handler.

**Gates:** the standard four (`pnpm install --frozen-lockfile`, `pnpm stack:up`, `pnpm verify`, `pnpm verify:full`). No browser suite is required — this stage has no UI.

**What to check in the Stage 12 report:** that `calendar.update`/`pattern.set`/`pattern.clear` all supersede rather than edit in place (G01, G04); that a newly created Entity gets an Active `WorkingCalendar` with no manual step, `Global`/`IN`/`SG` included (G02, F227); that an omitted `WorkingPattern` weekday inherits at read time rather than being defaulted at write time (G03); that `pattern.set`'s overlap guard is tested with a same-day replacement (allowed) and a backdated one (refused), not only the ordinary case (F229); that G05's location scoping is tested against employees in two different locations, not asserted; that `calendar.update`, `holiday.cancel`, `pattern.set` (when superseding) and `pattern.clear` each check `expectedVersion` first, and `holiday.add`/`holiday.confirm` take none (F228); that `calendar.update` copies forward the prior version's still-Active holidays and that a historical `calendar.get(asOf)` still resolves the old set (F230); that `holiday.confirm` writes no `VPS-F004` `AuditEntry` anywhere in the diff (F225) — grep for it; that no materialized index, cache table or precomputed row appears anywhere (F226) — grep the diff for anything resembling one; that `workingDays.*` is the sole resolution-order code path (G08); that the report does not claim to prove the PayRun/LeaveRequest/Contract exemptions — only the live-view default (F231); that no file under `apps/` appears in the diff; and that `holiday.importForJurisdiction` is not implemented.
...
Stages 1–11 are merged (Stage 11 as `96d53eb`). Stage 12 — Working Calendar and Working Patterns (`VRS-F004`) — is briefed above (Part 3), server-side only. F225 and F226 were ruled the day the brief was written; F227 through F231, raised tracing that brief before implementation (`fcfe213`), are now ruled too (F231 correcting F226's own overreach) and folded into the brief above. `VRS-001`'s Feature Register confirms it as the correct next MVP-order feature after `VRS-F003`. Resume on `codex/stage-12-working-calendar` (already at `fcfe213`, findings only, no implementation) on the fully corrected brief; move `FDN-119` back from Blocked to In Progress. When its report is written and Linear is updated, stop and wait for the founder, same as every stage boundary.
