...
A first attempt at this brief (branch `stage-9-shell-bootstrap`, now abandoned) stopped correctly on F217: F216's ruling assumed `session.activeOrganizationId` was already populated somewhere; it wasn't. F217 is now ruled too — the two steps it adds are folded into this version of the brief below (points 2 and 3). Start a **new** branch; don't build on top of the abandoned one.

**Linear:** file a new focused issue under Team FDN (same pattern as `FDN-103`/`FDN-104`/`FDN-114`/`FDN-115`), titled around "the real shell bootstrap (F216, F217)", linked to `FDN-69`'s milestone and citing both findings. Set it **In Progress** when you start. `FDN-114` (Stage 10's Reconnect issue) and `FDN-115` (the abandoned attempt) both stay as-is.

**The ruled contract (F216 + F217).** Checked directly against the code: `services/api/src/auth/config.ts` already configures Better Auth's `organization()` plugin server-side, and the session already carries `activeOrganizationId` — `requireCurrentWorkspaceSession` already treats it as the workspace claim, and `workspace-session.ts`'s revocation path already clears it to `null`. But nothing populates it: a repository-wide search found only that one write (the clear-to-null). `apps/roster-web/src/lib/auth-client.ts` also has no `organizationClient()` plugin, so nothing in the shell can read the field even once it holds something.
...
3. **A session → resolve the workspace and mount the real client.** Add `organizationClient()` to `auth-client.ts`'s plugins. Read `user.id` and `session.activeOrganizationId`. If `activeOrganizationId` is set, use it. If it's `null`, resolve it client-side: exactly one active membership → treat it as active (no real ambiguity); more than one, or none → this is unresolved workspace selection, not this stage's to invent — render a minimal holding state (plain copy is enough; this is **not** F214's `Reconnect` and must not reuse its trigger, copy, or component). Construct the real graph client (`createGraphClient`, the same one `/sync-harness` already exercises) with the resolved workspace and `user.id`, sending `x-vulto-workspace-id` per the existing convention (`packages/schema/src/mutations/define.ts`'s `WORKSPACE_HEADER`). Mount it in the shell layout, replacing `Shell.tsx`'s hardcoded `workspaceName` and `syncStatus` props with the client's real state.
4. **Offline boot stays offline, single-cache only (F217).** `VPS-A003`: "booting offline is restored." If there is no network, enumerate cached workspaces for this user. Exactly one `vulto:<workspaceId>:<userId>` cache → mount from it via `session_hint`, no network call. More than one → this is the same unresolved-selection case as point 3, not a silent pick; require coming online. Do not add anything to `device-identity.ts`'s allowed-key list — that database's narrowness is a deliberate, test-enforced contract, and F217 did not reopen it.
...
- A cold offline boot with exactly one cached workspace mounts from cache with no network call; more than one cached workspace does not silently pick and does not crash.
