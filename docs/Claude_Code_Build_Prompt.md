...
Stages 1–11 are merged (Stage 11 as `96d53eb`). Stage 12 — Working Calendar and Working Patterns (`VRS-F004`) — is briefed above (Part 3), server-side only. F225 and F226 were ruled the day the brief was written; F227 through F231 were raised and ruled resuming the brief a second time; F232 through F234, raised resuming it a third time (`bc21a52`), are now ruled too and folded into the brief above — AE's template corrected (F232), `entity.create`'s optimistic handler extended to match its server-side calendar write (F233), and reduced-hours periods given a write path through `calendar.update` (F234). `VRS-001`'s Feature Register confirms it as the correct next MVP-order feature after `VRS-F003`. Resume on `codex/stage-12-working-calendar` on the fully corrected brief; move `FDN-119` back from Blocked to In Progress. When its report is written and Linear is updated, stop and wait for the founder, same as every stage boundary.
