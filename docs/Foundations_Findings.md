...
This is the exact contract gap F223 resolved for `entity.deactivate` and F228 resolved for `holiday.cancel`, `calendar.update`, `pattern.set` and `pattern.clear`. Reading the current database version inside `validate()` would not prove which version the caller saw. Treating cancellation as an ordinary update would evade the concurrency rule. Adding `expectedVersion` changes the exact API contract in both the feature specification and the stage brief. No current ruling authorizes either choice.

The client half is unresolved for the same reason. Stage 13 Do item 7 calls `assignment.cancel` an ordinary node-write optimistic handler, but a corrected state transition would need to compare the cached Assignment's version first, reject `stale-state` on mismatch, and apply `Canceled` only against that checked version, following the existing Entity/Holiday/WorkingPattern handlers. Whether that is the intended contract must follow the server-side ruling rather than be invented independently.

**Status: open.** Stage 13 is blocked before implementation. A ruling must either correct the contract to `assignment.cancel(assignmentId, expectedVersion)` with version-first validation and matching optimistic behavior, following F223/F228, or explicitly amend A003-T54 for Assignment cancellation and state why this transition is exempt.
