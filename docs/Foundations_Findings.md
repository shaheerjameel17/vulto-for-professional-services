...
**Open decision.** Define how the shell distinguishes a confirmed absent session from an unreachable session service, and whether an offline cached `session_hint` may temporarily identify the user for boot. State what happens if caches belong to more than one user. Then amend the Stage 9 brief and the owning `VPS-A003`/`VPS-F001` clauses as needed. The builder made no such choice, changed no application code, and stopped with a BLOCKED report.
