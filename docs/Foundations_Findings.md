...
**Decision needed.** Stage 9 does not decide whether to expose Better Auth's read-only organization routes (and, if so, which ones and with what treatment of pending or inactive memberships), or to add a narrower authenticated endpoint that returns only the caller's active confirmed workspace IDs. Either changes the deliberate API boundary. The shell cannot infer the answer from `session_hint` on a device with no cache, and selecting an organization from all memberships without checking their state would contradict F217. `organizationClient()` alone is therefore insufficient. Correct the Stage 9 brief and the owning authentication specification (`VPS-F001`) with the chosen read contract before implementation resumes.

**Status: open.** Stage 9 stops here under the project decision policy.
