...
**The gap.** `VRS-F006` makes `RateCardLine` a fixed Tier 1 node whose substantive fields are `rate_card_id`, `seniority_level`, `hourly_rate`, `daily_rate` and `monthly_rate`. Stage 14 then requires two point lookups over those fields: `rateCard.getPreview(rateCardId, seniority)` and `resolveAssignmentRate`, which must find the line matching an Assignment's `rate_card_id` and its Employee's `seniority_level`. The same corrected spec explicitly says RateCardLine carries no edge of its own because nothing needs to traverse backward from a RateCard except "through the card's own lines," but it defines no representation of those own lines on RateCard and no lookup mechanism for reaching them.

The running server makes the missing path concrete. `services/api/src/graph/store.ts` retains only universal fields in `graph_nodes.record` for a node with no Tier 0 partition, so `rate_card_id` and `seniority_level` are absent from every stored RateCardLine graph row. They live only in the encrypted `record` partition in `graph_protected_fragments`. `protected.read` accepts node ids and optional partition names; it has no predicate or index over encrypted content. `VPS-A002`'s registry has no RateCardLine → RateCard edge, and the Stage 14 brief defines no deterministic RateCardLine id that could be derived from `(rateCardId, seniority)`. Therefore neither required lookup has an address it can ask the protected path to read.

**Why a workspace-wide decrypt-and-filter pass is not an implementation choice.** The only path available without changing the schema is to enumerate every RateCardLine universal row in the workspace, pass every id through the audited decrypt path, and filter the plaintext fragments in memory by `rate_card_id` and `seniority_level`. That is exactly the implicit foreign-key full scan `VPS-A002` A002-T03 prohibits; F243 was closed on the same rule when Assignment-side `rate_card_id` would otherwise have required a full Assignment scan. It also makes `VRS-F006`'s 200 ms preview NFR depend on the total number of every client's historical rate lines in the workspace rather than the selected card.

**Why this cannot be decided in code.** Several designs could make the lookup real, and each changes an authoritative contract: register and write a RateCardLine → RateCard edge; store line ids or the lines themselves in RateCard's protected partition; derive a stable line id from `(rateCardId, seniority)`; or add an explicit server-side encrypted-field lookup/index. They differ in graph traversal, erasure domains, versioning, mutation writes and query behavior. The current spec explicitly rejects the first without selecting any of the others.

**Status: open — blocks Stage 14.** The spec must define one addressable RateCard-to-line relationship before `rateCard.getPreview` or `resolveAssignmentRate` can be built without a prohibited scan or an invented schema.
