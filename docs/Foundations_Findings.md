...
| F205 | The Stage 2 brief makes `graph_nodes.created_at` `not null`, but `PulseAggregateContribution` and `WellnessAggregateContribution` are registered Tier 0 nodes that omit `created_at` by design (`VPS-A002` closed omissions; `VPS-A007` gate 3), because an exact creation time would let a contribution be correlated with a person | `VPS-A002`, `VPS-A007`, Stage 2 brief | **Open — blocks storing those two types.** Until decided, `insertNode` refuses them with a `GraphValidationError` naming F205 rather than inventing a timestamp. Every other node type is unaffected. Recommended: make `created_at` nullable, with a check constraint that it may be null only for the two anonymity-protected types |
...
**What the code does meanwhile.** The store fails closed: `insertNode` on either type throws a `GraphValidationError` that names this finding. The other 90-odd node types are unaffected.

**Recommendation.** Make `created_at` nullable and add `check (created_at is not null or node_type in ('PulseAggregateContribution','WellnessAggregateContribution'))`, so no other type can omit it.
