...
**One combination is recorded as unproven, in the open, per F143's precedent rather than silently left.** `entry.ts` prioritizes an `erased` report over a `discardedWrites` report when both are true. Whether both CAN be true simultaneously was checked structurally rather than assumed: `#endLocalSession`'s flush (step 1) runs while the device is still unlocked, before any lock has occurred, so it can only fail for a reason independent of the revocation itself — a genuine storage-layer fault (corruption, quota), not any path this application's own state machine can reach. No deterministic browser-level test could construct that combination without directly injecting an IndexedDB failure, which was judged not worth building for a three-line ternary whose worst case (reporting the wrong of two already-correct codes) is a labeling question, not a data-safety one. The ordering itself — erased wins, since it subsumes the discarded write — was still mutation-tested by temporarily reversing it and confirming the existing suite's cascading-revocation test would have caught a WRONG ordering if that specific combination were reachable.
