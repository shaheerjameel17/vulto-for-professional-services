...
**What was not decided.** The declaration is a reviewed registry entry, so it was not added. The mutation's logic was exercised by adding `governingPartitions: { Employee: "operational" }` to `managed_by` temporarily: all 19 pipeline tests passed, then the registry was restored by hand.

**Recommendation.** Declare `Employee: "operational"` on `managed_by` and `scoped_to_entity`. A reporting line and an entity scope are facts about a person's role in the organization, not about their compensation, the same reasoning `assignment_of` already carries. Then change `itF208` to `it` in `pipeline.integration.test.ts`.
