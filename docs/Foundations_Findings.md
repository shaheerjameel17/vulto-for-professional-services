...
**Not decided here.** Removing it means changing Better Auth's `member` additional fields and every query that reads it (the session guard, the principal resolver, the audience materializer, the shape proxy). That is a schema change to the authentication model and is left for a ruling.
