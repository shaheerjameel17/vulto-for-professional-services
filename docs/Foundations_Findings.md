...
**The gap.** The authoritative API contract and Stage 14 Do item 1 both name `rateCard.update(rateCardId, lines)`. The same Do item then says that `currency` is caller-supplied on both create and update, while the editor specification presents both name and currency as editable fields. The update contract accepts neither. The builder therefore cannot know whether a superseding card copies the prior name/currency, accepts a new currency only, or accepts both name and currency.

The version transition is also unresolved. `RateCard` is registered with `featureOwnedLifecycle`, and F223 records `is_active` as that convention's lifecycle field. Stage 14 says update marks the prior card `is_active: false`, but the mutation carries no caller base version even though `VPS-A003` A003-T54 requires one for every state transition and the brief says to mirror `calendar.update`'s supersession pattern. At the same time, G02 says the prior node is never edited in place, while setting `is_active: false` necessarily changes its protected record. Those statements can coexist only if the specification explicitly defines the lifecycle flag as the allowed exception; it currently does not.

**Why implementation cannot choose.** Copying the old currency would contradict the brief's caller-supplied instruction. Adding undocumented fields would change the public contract. Omitting `expectedVersion` would repeat the stale-state gaps already corrected by F223, F228, and F240; adding it would also change the contract. Leaving the prior card active would contradict Do item 1, while changing it without clarifying G02 would violate the immutability claim the reviewer is instructed to check.

**Status: open.** Stage 14 is blocked pending one corrected update signature and explicit rules for copied versus caller-supplied fields, version-first stale-state handling, and the prior card's permitted lifecycle-only change.
...
**The gap.** Stage 13's `assignment.create` is Tier 0, `onlineOnly: false`, and has a real optimistic handler. Stage 14 now requires that same mutation, when `rate_card_id` is supplied, to read a Tier 1 RateCardLine through the protected path, resolve `effective_billing_rate`, and write a `governed_by` edge whose RateCard endpoint is never present in the device cache. F244 correctly forbids caching either RateCard or RateCardLine. Stage 14 item 7 precisely defines the three new Assignment-rate optimistic handlers but says nothing about the existing `assignment.create` handler whose authoritative effect item 2 changes.

**Why the existing mechanisms do not answer it.** `MutationDefinition.onlineOnly` is static for a mutation name; it cannot make only card-bearing creates online-only while preserving offline creation without a card. The device cannot resolve the line, validate the Tier 1 target, or safely invent the new effective rate. It also cannot “leave the previous value unchanged,” as `setRateCard` may, because a newly created Assignment has no previous row or rate. The available choices therefore have different product behavior: make every Assignment create online-only, add a conditional connectivity rule, split card selection into a later `setRateCard`, or define an explicit provisional local effect.

**Status: open.** Stage 14 is blocked until the contract states whether card-bearing Assignment creation is allowed offline and exactly what its optimistic node and edge effect is when protected RateCardLine data and the RateCard node are absent from the device.
