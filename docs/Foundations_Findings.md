...
| F149 | A purged Worker reports itself uninitialized to a caller already in flight, with no render defined for that moment | `VPS-D004`, Repository | **Open, named follow-up** — filed on FDN-63; no product screen consumes the locked state yet, so no live consequence today |
...
| F167 | A003 requires Tier 1 recovery to use a pinned, audited, standards-conformant SLIP-0039 implementation while prohibiting hand-written Shamir, but the identified browser JavaScript SLIP-0039 implementation explicitly disclaims production maturity and publishes no audit; the audited browser alternative is not SLIP-0039 | `VPS-A003`, Repository, Dependency audit | **Open, blocks FDN-52 closure** — the founder prohibited weakening A003 and split Stage 6 so non-recovery work could close. Stage 6B found no already-audited Worker-compatible implementation and recommends `shurlinet/go-slip39@5c07db3111d767e326933838ac92d7c5b3d8ef15` only as the complete shipping-unit audit target. No Tier 1 recovery code or candidate dependency is production-reachable; recovery remains blocked until that exact source/compiler/dependency/WASM/adapter unit passes independent audit and explicit approval |
...
**Open and independently blocks FDN-52's direct implementation closure.** No Tier 1 recovery code or production dependency was added. Recovery remains disabled until the exact shipping unit is audited, findings are closed, its pinned artifacts are approved, and the repository/toolchain addition is explicitly accepted. Official-vector and bidirectional Trezor-Python interoperability are mandatory gates but are not substitutes for that audit. F172–F175 were independent, not subparts of F167, and are now closed by their own rulings and proof.
...
### F184 — corrected after reading the source audit reports directly: the shipped `privy-io/shamir-secret-sharing` reintroduces the one defect either audit ever rated High

Raised assessing `privy-io/shamir-secret-sharing` as a direct F167 candidate, once F183 was decided (no requirement that Tier 1 recovery be portable outside Vulto's own software). **This entry originally recommended `v0.0.4` for pinning. Having now read both source audit PDFs directly rather than Privy's own summary of them, that recommendation is withdrawn as stated.** The two paragraphs below replace the corresponding claims in the version of this entry the founder was shown before ordering the reports read; they are not a caveat added alongside the original claims, because the original claims do not survive contact with the primary sources.

**What it is, verified from source and the registry rather than from its own README's summary line.** `shamir-secret-sharing`, published by Privy, Apache-2.0, zero runtime dependencies. The polynomial arithmetic in `src/index.ts` is textbook GF(2⁸) Shamir secret sharing using lookup tables sourced from HashiCorp Vault's own implementation — a standard, well-known construction, not an approximation and not bespoke math. `split(secret, shares, threshold)` and `combine(shares)` operate on arbitrary-length `Uint8Array` byte strings, which fits a 256-bit secret directly. The browser build's randomness (`src/csprng.ts`) calls `crypto.getRandomValues` — the standard Web Crypto API already used everywhere else in Vulto's cryptographic Worker, with no Node-only API and no WASM/Go toolchain required. This much still holds; what follows does not.

**What the two reports actually say, read directly rather than through Privy's blog.** Cure53 audited commit `3383ad9` (a pre-release beta, January 2023) over two person-days and reported three findings: `PVY-01-002`, rated **High** — the leading polynomial coefficient could be zero, and Cure53's own report states the consequence in plain terms: *"t−1 parties — which is one fewer than expected as a demand — can jointly recover at least one byte of the secret, hence learning something from said secret."* Cure53 computed this for a concrete case matching ours in kind: splitting an AES-128 key (16 bytes) across repeated calls, they calculate roughly 4 executions in 100 will expose at least one full byte to below-threshold holders. All three findings — PVY-01-002 plus two Info-level items (an interpolation inefficiency; the lookup tables' non-resistance to cache side-channel attacks) — are marked *"fixed and verified by Cure53."* Zellic separately audited commit `cd8422d` (tagged `v0.0.2`, June 2023, **after** Cure53's fix was already in the code) over two person-days, with an explicit two-day time-box and "front-end components" and "infrastructure" both out of scope. Zellic reported exactly **one** finding — Medium severity/Low likelihood, missing 0–255 bounds checks in the raw arithmetic functions, unrelated to coefficients or sampling — rated Low overall impact, acknowledged and fixed. **Zellic's report contains no mention of the leading-coefficient question, uniform sampling, perfect secrecy, Ghodosi, or Cypher Stack, anywhere.**

**The claim this entry previously repeated — that Zellic and Cypher Stack found the fix itself flawed, and that Cure53 publicly reversed its own recommendation in April 2024 — does not appear in either audit deliverable.** It exists only in Privy's own engineering blog post, which is not an independent, dated, reviewable document the way the two PDFs are. Having now read both PDFs directly, I cannot corroborate that a third-party review of any kind ever examined, endorsed, or was even shown the reverted construction that ships today.

**And the revert is real, checked directly against every tagged release.** `newCoefficients()` in `v0.0.1`, `v0.0.2` and `v0.0.3` all enforce a non-zero leading coefficient — Cure53's exact required fix, present from the first tagged release onward. `v0.0.4` (commit `3333451`, tagged and published to npm 10 January 2025) removes that enforcement and returns to plain uniform sampling — reinstating, byte for byte, the condition Cure53's own report rated High and required fixed. **The version this entry recommended pinning is the version that undoes the one thing either formal audit ever flagged as a security vulnerability, and neither audit has reviewed the undoing.**

**Quantified for our exact case — the numbers stand, the conclusion they support does not.** For a 256-bit Tier 1 recovery secret under the 2-of-3 default, each byte is shared by a degree-1 polynomial with one random coefficient `c` uniform over GF(2⁸). P(a given byte's `c = 0`) = 1/256 exactly. P(at least one of 32 bytes degenerate per split) = 1 − (255/256)³² ≈ **11.77%, roughly 1 split in 8.5** — matching Cure53's own AES-128 calculation in kind, scaled to our longer secret. An earlier draft of this paragraph additionally claimed that below-threshold shares reveal *nothing* about a degenerate byte even when one occurs, reasoning that detecting degeneracy requires two points, which is already at-threshold. **That derivation is wrong, and Cure53's own report is the reason I know it: the below-threshold reconstruction does not need to detect degeneracy in advance — running the standard reduced-degree interpolation directly on t−1 points produces the correct byte value with certainty whenever that byte truly is degenerate, no verification step required for the computation itself to be correct.** My own reasoning, unchecked against the primary source, produced a confident and wrong conclusion; the source contradicts it directly and explicitly, in the plainest terms an audit report uses.

**What this means for the actual recommendation.** `v0.0.1` through `v0.0.3` cannot exhibit this issue at all — the non-zero enforcement makes P(degenerate) = 0 by construction, at zero cost, for every split. `v0.0.3` specifically (16 August 2023) is the one point in this library's published history where **every finding from both formal audits is fixed and none has since been reverted**: it carries Cure53's non-zero-coefficient requirement (present since `v0.0.1`) and adds Zellic's exact bounds-check fix in the same release (commit `1c51059`, "Add 0-255 uint8 bounds checking to arithmetic functions"). `v0.0.4` is not that version. If this library is used at all, `v0.0.3` is the version with an actual, checkable, unretracted audit trail behind it — not `v0.0.4`, and this is a new recommendation this entry had not previously made, itself unvetted to the standard the rest of this project holds (I have not, for instance, independently confirmed nothing else changed between `v0.0.3` and `v0.0.4` beyond the coefficient revert and the two dependency-manifest files — a fuller diff review, not just the file list already pulled, is owed before anyone pins anything).

**Vulto's downstream AEAD does not rescue this, and citing it here was the wrong move on my part.** It's true that the reconstructed secret must still authenticate against AES-GCM before it unwraps anything, and that a lucky one-byte guess doesn't let an attacker test bytes independently against that tag. But Cure53 rated PVY-01-002 High as a **formal violation of the threshold scheme's own contract**, independent of what any particular downstream consumer does with the output — that is exactly what a cryptographic library audit is for, and pointing at a consumer-side mitigation to justify shipping a construction the audit already required fixed inverts the right order of operations. The correct response to a formally verified High-severity defect that has a zero-cost fix sitting in the same repository's own history is to use the fixed version, not to argue the defect is survivable downstream.

**What I could not verify myself, stated plainly.** I have now read both PDFs' text directly via `pdftotext`, not through a summarization layer — this closes the gap the previous version of this entry named. What I have *not* done: independently confirmed there is no THIRD, later audit of `v0.0.4` specifically that simply wasn't surfaced by search (a negative is hard to prove); performed the full file-level diff between `v0.0.3` and `v0.0.4` beyond the coefficient-generation function and the two manifest files already checked; or evaluated whether Privy's own underlying cryptographic argument (the Ghodosi-style repeated-split concern) is itself sound, since it appears nowhere in either report I could check it against.

**Fit against A003-T13/T14.** T13 (product-generated, labeled, correct re-entry before completion) is a ceremony requirement the library has no opinion on; Vulto's own code supplies it, using the identical pattern already built and proven for the Tier 3 recovery-code envelope. T14 (M-of-N, 2-of-3 default) is exactly what `split`/`combine` implement natively.

**Composability: zero new primitives.** The reconstructed secret feeds the *same* pipeline A003's Tier 1 recovery envelope row already specifies — HKDF-SHA-256 over the recovered secret, AES-256-GCM, the RFC 8785 canonical header as AAD. The only thing this candidate changes is which code performs "split a 256-bit secret into N shares / combine M of them" — a single step in an already-specified pipeline, not a new architecture.

**The library's stated reconstruction-verification limitation is separately covered by Vulto's own design, and this part of the assessment is unaffected by the correction above.** Its README states it "does not verify the result of share reconstruction" — a caller feeding in wrong or corrupted shares can get back a wrong value with no signal. Vulto's architecture already wraps everything downstream in authenticated encryption: a wrongly reconstructed secret used as HKDF input key material produces a key that fails to authenticate the AES-GCM ciphertext it is asked to decrypt. This is a real, independent point in the library's favor regardless of which tagged version ships — it is the *coefficient* question above, not this one, that changes the recommendation.

**No combinatorial limit, unlike the pairwise-envelope alternative from the earlier F167 memo.** Genuine Shamir needs exactly N shares regardless of M, at any N. The pairwise scheme needed C(N,M) envelopes and stopped being practical around N≈6; this needs 10 shares at N=10 and 5 shares at N=5, the same as N=3. If this candidate is approved, the pairwise alternative should be abandoned in its favor — it is strictly better on every axis except one.

**What it does not give.** No transcribable mnemonic word-list format, no cross-vendor interoperability with SLIP-0039 tooling such as Trezor's. That is the exact trade F183 already ruled acceptable — Vulto's own software remains the only thing that can reconstruct the secret either way, since this candidate's raw byte-string shares are not SLIP-0039's wire format either.

**Proposed A003-T14 amendment: withdrawn as previously worded, not merely revised.** The prior draft named `v0.0.4` as the pinned artifact. That version is not recommended for pinning by this entry any longer. If the library is approved at all, the amendment would need to name `v0.0.3` specifically — pinned by exact commit, not by "latest" or a floating tag — with the reasoning above (both audits' fixes present, neither reverted) carried into A003's own text rather than left in this findings entry alone, so a future reader of the spec sees why that exact version and no later one is named.

**The design note about never re-splitting the same secret still holds, but is no longer load-bearing for this entry's recommendation.** It remains good practice regardless of which tagged version is used. It is not, on its own, a reason to prefer `v0.0.4` — `v0.0.3` needs no such compensating practice, because it cannot exhibit the coefficient issue at all.

**F167 stays open. Not recommended for closure — for a stronger reason than before.** The previous version of this entry said F167 should stay open only because I had not yet read the audit reports myself. Having now read them, F167 stays open for a substantive reason: the artifact this entry recommended pinning is not the artifact either audit reviewed and approved, on the one question either audit ever raised a security finding about. This is exactly the outcome the founder named in advance as the one to avoid getting backwards — a confident "we can avoid the audit" that does not survive reading the primary sources. It didn't survive. Recorded here rather than smoothed over, per this project's own standing practice for a self-correction.
