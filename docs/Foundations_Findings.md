...
| F260 | F259 requires the local `conflictCheck.evaluate` result to include `callerCanEdit` and the optimistic `overrideAndProceed` handler to admit a Manager or Owner, but the corrected query input carries no caller identity or roles, `MutatorContext` carries only `userId` and no roles or membership, and `OptimisticCache` cannot enumerate membership nodes | `VRS-F008`, Stage 16 Do items 4/9, `packages/graph/src/mutators/foundation.ts`, `packages/graph/src/mutators/cache.ts`, `packages/graph/src/queries/cache-data.ts`, F259 | **Open.** A target employee's manager can be inferred from locally cached `managed_by` plus Employee `user_id`, but Owner status and the role-derived `callerCanEdit` result cannot. The brief must define how caller identity and roles enter both local execution paths before product code can choose an authorization contract |
