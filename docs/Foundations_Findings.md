...
Stage 15 resumed on F251 and exposed open F252 while tracing its required audit call, still before product code: **198 numbered rows, F54–F252 with one gap (F198): 183 closed, nine open (F70, F71, F73, F85, F91, F118, F125, F129, F252), and six recorded facts or limitations (unchanged).**
...
**Decision required.** The ruling must choose one supported contract: remove or defer the successful-promotion audit requirement under `VPS-F004` G04; extend the centralized interceptor and closed audit taxonomy with a suitable successful-operation event; or identify an existing event and define how it can truthfully represent this Tier 0 promotion without a feature calling `appendAudit` directly. Until then, implementing Do item 5 would require contradicting either `VRS-F007` or `VPS-F004`.
