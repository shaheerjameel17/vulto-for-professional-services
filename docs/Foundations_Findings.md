...
| F184 | `privy-io/shamir-secret-sharing`'s currently shipping version (`v0.0.4`) reverts the one fix either formal audit ever rated a security vulnerability (Cure53, High, quoted directly); `v0.0.3`, fully vetted at its exact commit and npm integrity hash, carries every audited fix with none reverted | `VPS-A003`, Linear | **Recommended closeable on `v0.0.3` pinned by commit and integrity hash** — not `v0.0.4`; F167 remains formally open pending founder ruling |
