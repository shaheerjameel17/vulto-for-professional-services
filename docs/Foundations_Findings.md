...
**Status: open — Stage 11 blocked during implementation.** The existing Stage 11 branch keeps its partial server mutations, founding record, permission row, and focused tests; it is not reset. The focused schema tests and two API integration files passed against Postgres. `pnpm verify` currently stops at the missing optimistic mutators, and `pnpm verify:full` cannot yet be run meaningfully. Decide where the one temporal-resolution implementation lives and what the device's Tier 0 optimistic handlers must do, then correct the owning spec and Stage 11 brief before work resumes.
