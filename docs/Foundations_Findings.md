...
| F254 | The registered `promoted_to` edge targets tier-split Employee but declares no governing partition, so the permission interceptor refuses every promotion before its transaction can run | `VPS-A002`, `VRS-F007`, Stage 15 Do item 4, `packages/schema/src/registry/edges.ts` | **Open, raised not decided** — FDN-122 blocked |
...
Stage 15 resumed on F252 and exposed open F253 while tracing the two-node create into the required Employee fields, still before product code: **199 numbered rows, F54–F253 with one gap (F198): 184 closed, nine open (F70, F71, F73, F85, F91, F118, F125, F129, F253), and six recorded facts or limitations (unchanged).**
...
**Status: open.** Stage 15 is blocked before completing product code. A ruling must state which Employee partition governs `promoted_to`, update `VPS-A002`'s governing-partitions table and the live edge registry together, and extend Stage 15's Do item/Done criteria accordingly. If the ruling selects `operational`, the implementation is the same one-line registry declaration used by F208/F235, after which the already-written integration test can prove the promotion transaction for real.
