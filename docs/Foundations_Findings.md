...
| F206 | `VPS-A004` says support and system principals are evaluated against their own policy rows and every decision they receive is audited (A004-T22), but nothing defines the rows or the audit shape: `VPS-F004`'s `AuditEntry` requires `actor_user_id` and `actor_membership_id` and names no way to record a support grant or a named system job, `VPS-A008` describes a support scope only as "which workspace areas, read or write", and no document lists which operations each system principal may perform | `VPS-A004`, `VPS-F004`, `VPS-A008` | **Open — blocks auditing and evaluating support and system principals.** Members are complete. Until decided, a support or system principal resolves to no node grant and an attempt to audit one throws, so nothing is released unaudited. The one system operation that has code today, `erasure` pseudonymizing an audit actor, is a policy row. Recommended below |
...
**Recommendation.**
1. `AuditEntry` gains `actor_kind` (`member`, `support`, `system`). `actor_user_id` and `actor_membership_id` stay required for a member. A support entry carries `actor_grant_id`; a system entry carries `actor_system_name`, both from the closed vocabulary. `audit_journal` gains the same columns, with the actor index keyed on whichever identifies the actor.
2. A support scope is `{ access: "read" | "read-write", node_types: NodeType[] }`, evaluated against the same policy table as a member, with the grant capped at the scope and never above Owner. A008 "areas" means node types.
3. Each stage adds a system operation to the policy table when it builds the job that performs it: Stage 5 adds `key-rotation` and `erasure`'s key destruction, Stage 6 adds `audience-recompute`.
