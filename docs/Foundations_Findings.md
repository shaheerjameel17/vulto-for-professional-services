...
| F135 | `VPS-A004`'s per-decision `AuditEntry` requirement (`VPS-F004`) was never surfaced scoping FDN-53 stage 2, and no audit-writing code exists anywhere in `packages/graph` | `VPS-A004`, `VPS-F004`, Repository | **Closed by FDN-68 Stage 8.** The centralized read/mutation interceptors now write the canonical event contract through the atomic sealed journal/outbox before sensitive release, with authenticated server delivery and permission-aware retrieval. The complete real Chromium/PostgreSQL audit suite and all twelve adverse mutations prove the boundary |
...
| F197 | FDN-85's reconciliation trail is a bespoke structured log, not a typed audit record — FDN-68's audit interface does not exist yet, and building a one-off `AuditEntry` writer here would repeat the mistake F132/F133/F135 each avoided (inventing infrastructure under stage pressure) | Repository, FDN-68 | **Closed by FDN-68 Stage 7; re-proved in Stage 8.** `commitPrivilegedProjection` records one canonical `PrivilegedProjectionAuthorized` command event through the same typed recorder, sealed journal/outbox, server journal and query API as every other audit event. The reconciliation outbox retains operational delivery state only; its bespoke `authorizationPath` marker is removed. Deterministic command identity makes a logical replay add no second event. The complete browser matrix retains the Foundation/Roster proof with distinct registered applications and no feature-level writer |
| F198 | AuditEntry is registered as `immutable-audit`, and VPS-F004 makes its `Full` grant read-only, but the current generic mutation gate does not reserve it: Owner/HR Admin resolve to ordinary `full`, every changed non-projection node fragment reaches `authorizeNodeWrite`, and the AuditEntry schema's passthrough content remains mutable even though update/soft-delete bookkeeping is forbidden | `VPS-F004`, Repository, FDN-68 | **Closed by repository fix, independently verified by the founder and mutation-tested in Stage 8.** The gate reserves AuditEntry ahead of `authorizeNodeWrite` while leaving its read policy unchanged. Chromium/Postgres/sealed-store proof refuses Owner and HR Admin create, alter, remove, and a genuine AuditEntry → Skill type change; nothing forbidden survives reopen and an ordinary mutation still commits. Removing the reservation changes the Owner create result from `unsupported` to `applied`, killing the proof |
...
**Final closure by FDN-68 Stage 8.** The separate system this finding deliberately deferred now exists at the centralized boundary: every-tier denials, Tier 1 grants, authorized failures, and the distinctly authorized privileged-projection command use one closed `AuditEntry` contract and one internal recorder. The sealed local journal and durable outbox are written atomically before sensitive results are released; server delivery is authenticated, workspace-isolated, idempotent, and historically queryable only by Owner or HR Admin. The real Chromium/PostgreSQL acceptance suite proves denial non-enumeration, non-recursive audit-query refusal, offline recovery, failure closure, local/historical paging, pseudonymization invariance, and representative Foundation/Roster evidence. Each of the twelve approved adverse mutations was detected by a named real-stack assertion and then reverted. Audit records never enter the generic Loro/SQLite graph path, and the repository still has no VPS-F002 search implementation; current search exclusion is therefore structural, while VPS-F002 retains responsibility for proving its future index also excludes this separately stored journal.
...
* it **bypasses per-write role authorization** because the command's own gating is the authorization: `createPendingWorkspaceAdmission` already succeeded server-side, the grant is single-use and hash-verified, and the projection command is structurally incapable of writing anything but the reserved-type records (it takes typed scalars, not fragments). This is a distinctly named path — `commitPrivilegedProjection`, not a flag on `authorizeNodeWrite` / `authorizeEdgeWrite` — and FDN-68's canonical audit recorder records the exception as `PrivilegedProjectionAuthorized` with `authorization_path: "PrivilegedProjectionException"`, never indistinguishable from an ordinary authorized write. **The exception covers this issue's projection command only; any future privileged write needs its own ruling.**
...
**Closed by FDN-68 Stage 7.** Audit evidence now lives at the command boundary inside `commitPrivilegedProjection`, so both founding admission and ordinary-session role-change/revocation call sites are covered by construction. One logical command emits one `PrivilegedProjectionAuthorized` event through FDN-68's canonical schema, sealed local journal/outbox, authenticated workspace-isolated server journal, and permission-aware query API. Its deterministic identity is derived from workspace, membership, projection kind, and normalized occurrence time; an identical replay adds no event, while reuse with different logical content hard-conflicts.

The FDN-85 outbox remains only an operational delivery/reconciliation record (`kind`, membership, workspace, creation time, local durability, server confirmation). Its bespoke `authorizationPath` field is removed, so it cannot remain as a second permanent audit format. Real Chromium/PostgreSQL proof exercises the server-signed founding projection and a Roster Tier 1 Employee-compensation read through the same central journal and server table, distinguishes their registered `VultoProjects` / `VultoRoster` application identities, and proves the feature-level projection and Roster fixture sources contain no recorder construction or `.record()` call.

**Stage 8 closure re-proved this finding in the complete browser matrix.** The representative integration remained green alongside every other device-store scenario, and the canonical event path remained covered by the same schema, sealed journal/outbox, authenticated server journal, and query API used throughout FDN-68. No feature-level audit writer or surviving bespoke authorization marker was found.

---

### F198 — AuditEntry falls through the generic mutation gate despite its read-only Full exception

Recorded during FDN-68's dedicated design pass and independently verified by the founder against the repository rather than accepted as a self-reported code reading.

`packages/graph/src/worker/permission/policy-table.ts:611` gives Owner and HR Admin an ordinary `full` outcome for AuditEntry. `packages/graph/src/worker/permission/mutation-interceptor.ts:472` reserves only Workspace/WorkspaceMembership and their projection edges; at line 490 every other changed node fragment reaches `authorizeNodeWrite`, whose ordinary rule grants any `full` outcome. `packages/schema/src/records.ts:145` forbids AuditEntry's update and soft-delete bookkeeping fields but ends in `.passthrough()`, so it does not make the remaining event content immutable. An otherwise coherent generic delta can therefore create, change or remove AuditEntry content for an Owner or HR Admin, contrary to VPS-F004's explicitly decided read-only interpretation of Full.

**Closed by repository fix, independently verified by the founder.** FDN-68 Stage 1 reserves AuditEntry from generic mutation ahead of `authorizeNodeWrite`, and leaves the Owner/HR Admin read grant untouched. The real Chromium/Postgres/sealed-store proof exercises Owner and HR Admin create, alter and remove, verifies that no forbidden mutation lands durably after reopen, and proves that an ordinary permitted Skill mutation still commits in the same runtime. A dedicated scenario also sends an otherwise-valid AuditEntry → Skill rewrite through the real `mutate()` gate, receives the same `unsupported` refusal, and proves the original node type and event content remain unchanged. With the reservation temporarily disabled, the browser proof receives `applied` for the forbidden create; restoring it returns the suite to green. The internal append writer and narrow pseudonymization path remain later FDN-68 stages, not part of this finding's generic-mutation closure.

**Stage 8 final state.** The complete device-store browser suite retains the create, alter, remove, and genuine AuditEntry → Skill type-change proof for Owner and HR Admin. The approved reservation-removal mutation was killed by the Owner create assertion (`unsupported` became `applied`) and then reverted. The canonical append and pseudonymization seams are now implemented separately and expose no generic AuditEntry mutation capability, so the finding remains closed without weakening the read-only Full grant.
