...
**Status: open.** Stage 12 is blocked on one ruling: either declare `governingPartitions: { Employee: "operational" }` for `pattern_for` and record it in `VPS-A002`, following F208, or define a different authorized relationship contract. The partial Stage 12 implementation remains on `codex/stage-12-working-calendar`; no registry workaround has been added.
