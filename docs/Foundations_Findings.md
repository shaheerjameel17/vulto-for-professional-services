...
### F216 — the Reconnect state has no session-aware shell call to retry

Raised 22 September 2026 while tracing Stage 9 before implementation. `VPS-D004` requires a call that had a session and was refused to show *Reconnect*, a call with no session to go to sign-in, and **Retry** to reissue the exact refused call. It explicitly requires the same treatment at cold start and mid-session. Stage 9's browser suite must demonstrate both sides with a real `401`.

The current `apps/roster-web/src/app/(shell)/layout.tsx` only wraps the fixture `Shell`; `Shell.tsx` makes no authenticated call, starts no graph client, and displays a fixed `Synced` status. The only application mount of `createGraphClient` is the standalone `/sync-harness` test route, which gets `workspaceId` and `userId` from query parameters. Better Auth's `useSession` appears on `/auth-ready` and `/devices`, both outside the shell; neither provides the shell with a prior-session marker or a workspace when the server refuses the session. The sync client's `session_hint` holds identifiers inside the SQLite cache and has no shell-facing discovery or read contract. A cold start after session expiry therefore has no specified way to know which authenticated call to issue, or whether a resulting `401` belongs to a device that had a session.

The existing shape path also cannot implement **Retry** as written by simple wiring. `shape-source.ts` classifies `access-revoked`; `SyncEngine.#handleRevoked` erases the workspace cache and stops the engine, then publishes `signedOut: true`. `SyncState` carries the revocation reason but exposes no refused-call retry operation. A plain shape `unauthenticated` publishes the ordinary `Offline` status, and a tRPC `unauthenticated` from `protectedRead` becomes `requires-connection`; neither surfaces the refusal to the shell. These are observations about the current interfaces, not requests to change the Stage 6 erasure decision.

**Decision needed in the owning specifications.** Name the authenticated shell bootstrap call and the authority for the workspace and user identifiers at cold start, including what prior-session evidence distinguishes sign-in from *Reconnect* after the server refuses. Define how that call coexists with `VPS-A003`'s offline boot, how its exact retry survives a stopped worker and an erased cache after `access-revoked`, and which client boundary delivers these two refusal signals to the shell. A solution that adds another persisted marker or chooses a workspace from several caches has privacy and cross-workspace consequences, so Stage 9 cannot choose it as an implementation detail. Keep the ruled triggers, copy, and erasure behavior of F214; this finding concerns the missing bootstrap contract that makes them executable.

**State:** Open. FDN-114 and Stage 9 are blocked on this ruling. No Reconnect UI or trigger wiring was built.
