...
### F241 — Bench aggregate locality and server disclosure control define incompatible API contracts

Raised 23 September 2026 while resuming Stage 13 (`FDN-120`) on F240's ruling, before any product code was written.

**The conflict.** `VRS-F005`'s API contract defines one `benchForecast.get(workspaceId, window, filters?)` result containing `rows`, `aggregateUtilization` and `cohortSize`, then states that this call resolves entirely from the device's Electric-replicated cache with no network round trip. Its offline acceptance criterion and 50ms local-filter requirement reinforce that reading. Stage 13 Do item 3 likewise defines the local-cache query, but narrows its result to bars, dates and bench-day counts. Do item 4 then requires aggregate utilization and filtering to be computed server-side and returned from “`benchForecast.get`'s aggregate portion,” because disclosure control needs a single enforcement point.

There is no server route named `benchForecast.get`, no separate aggregate endpoint, and no input or response contract describing how the local rows and server aggregate are split or merged. `benchForecast.getCost` cannot carry it: that endpoint has a separate compensation-derived purpose and response shape, and the brief explicitly calls aggregate utilization another server-side call. Keeping the aggregate local would contradict item 4's server-only enforcement. Adding a new `benchForecast.getAggregate` route, moving all of `benchForecast.get` to the server, or silently removing `aggregateUtilization` from the documented local result would each invent a materially different API and offline behavior.

F238 resolves the disclosure algorithm and threshold ownership, but it does not resolve where its first caller runs. Its ruling still names `VRS-F005`'s `benchForecast.get` as the caller without correcting the local-only contract, while the later Stage 13 brief explicitly places the caller on the server. F236 split cost and Tier 2 panel data into named server calls, but made no corresponding split for aggregate utilization.

**Status: open.** Stage 13 is blocked before implementation. A ruling must choose the authoritative boundary: either aggregate utilization remains in the local `benchForecast.get` contract and the brief's server-side requirement is amended, or it moves to a specifically named server endpoint with defined inputs, response shape, offline behavior, and composition with the local rows. The specification and Stage 13 brief must then be corrected together.
