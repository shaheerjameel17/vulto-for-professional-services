...
### F191 — explicit device revocation retires the canonical Device identity everywhere, not per workspace

Surfaced building FDN-63 Stage 3, wiring the new `device` table into the merged F151 revocation signal.

The merged F151 slice classified `device-revoked` from `device_unlock_secret.revokedAt` — a per-`(workspace, device)` row — so an explicit single-device revocation was workspace-scoped, and its browser test proves "an explicit single-device revocation erases only that device's workspace." FDN-63's canonical `device` table has one row per `(user, application)` and one `is_revoked` boolean: there is no per-workspace dimension on it to preserve that scoping.

Two readings of `VPS-F001`'s "An Owner revokes a device… The Device node is marked revoked":

* **Per workspace.** The Owner of workspace X can only speak for workspace X; a device shared with workspace Y keeps that access.
* **Global retirement.** The device is flagged untrusted, full stop.

**Decision: global retirement.** The Device node is a single global identity, `VPS-F001` marks *it* revoked, and every realistic reason an Owner clicks Revoke — lost, stolen, an ex-employee's laptop, a compromised browser — is a reason the device should hold no workspace's data. The Modal confirmation `VPS-F001` mandates ("destructive, irreversible from the user's side") is the safeguard against a mistaken click, not per-workspace narrowing. A genuine multi-workspace consultant whose device one client revokes registers a fresh device (new generated id) for the others; the revoked identity stays revoked.

The per-workspace `device_unlock_secret.revokedAt` write is kept in `revokeDevice` alongside the new `device.is_revoked` write, so the merged F151 tests — which assert on the secret — keep testing a real code path. The role-refresh classifier now treats either fact as `device-revoked`.
