...
Stage 15 resumed on F250 and immediately exposed open F251 before product code was written: **197 numbered rows, F54–F251 with one gap (F198): 182 closed, nine open (F70, F71, F73, F85, F91, F118, F125, F129, F251), and six recorded facts or limitations (unchanged).**
...
**Decision required.** A ruling must choose one authoritative permission contract: retain Stage 13's explicit workspace-staffing overrides (and correct Stage 15's zero-row/fallback claims), change those overrides to let Manager create Ghosts, or remove them and define a real subject/scope rule that produces the intended access. It must also reconcile the Manager-create flow with the chosen Manager grant. Stage 15 cannot safely implement or test its named mutations until the interceptor's expected answer is settled.
