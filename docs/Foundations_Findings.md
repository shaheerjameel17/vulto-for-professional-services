...
| F209 | `VPS-A003` says `VPS-F004` records that a cryptographic erasure occurred, and the Stage 5 brief requires the erasure call to `appendAudit`, but the `AuditEntry` vocabulary has no event type, operation or target for it: the event types are `PermissionDenied`, `SensitiveAccessGranted`, `AuthorizedOperationFailed` and `PrivilegedProjectionAuthorized`, the operations are all node and edge reads and writes, and the targets are node, edge and query | `VPS-F004`, `VPS-A003`, `VPS-F007` | **Open — blocks the audit of an erasure.** Key destruction is built and tested with an explicit `ErasureAudit` seam whose default refuses, so in production no key is destroyed until this is decided. Recommended in the finding |
...
**What was built without deciding.** `eraseErasureDomain` writes the audit entry first and destroys nothing if that fails. The entry goes through an `ErasureAudit` seam whose default throws `ErasureAuditUndefinedError`, so as shipped no key can be destroyed. The tests inject a recording audit to prove destruction, isolation between domains, cache eviction and that an erased domain reads as `erased` and is never re-created.

**Recommendation.** Extend `AuditEntry` with event type `CryptographicErasureExecuted` (outcome `Granted`), operation `KeyDestroy`, and a target kind `ErasureTarget { erasure_domain_id, tier, key_count }`, written by the `erasure` system principal (`actor_kind: system`). The target holds identifiers and a count, never content.
