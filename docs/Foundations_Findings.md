...
**Status: open.** Stage 16 is blocked after product implementation began. The local query, optimistic handler, shared server aggregation and Owner-authorized server path are implemented, but the authoritative Manager path cannot satisfy the current permission contract.
