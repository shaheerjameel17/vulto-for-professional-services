...
### F242 — The server aggregate endpoint has no filter contract

Raised 23 September 2026 while resuming Stage 13 (`FDN-120`) on F241's ruling, before any product code was written.

**The gap.** F241 establishes `benchForecast.getAggregate(workspaceId, window, filters?)` as the authoritative server call for filtered aggregate utilization. `VRS-F005`'s Filtering section lists skill, seniority, department, entity and availability window as user-facing concepts, but neither its API contracts nor Stage 13 defines the `filters` object. There are no field names, value types, empty-filter behavior, within-filter or across-filter combination rules, or normalization rules. The availability filter is especially undecided: “available in a window” could mean at least one bench day, wholly unassigned throughout, available on the first day, or available by the end date, and those produce different cohorts and aggregates.

The graph traversal is also unspecified. Skill matching could require all selected skills or any selected skill through `has_skill`; Entity matching must resolve the temporal `scoped_to_entity` edge for the visible window; seniority and department can use Employee fields but have no stated multi-value semantics. The local `benchForecast.get` row query and the server `getAggregate` endpoint must use the same filter contract or the displayed rows and reported cohort size will disagree.

Passing a list of locally matched employee IDs is not an authorized workaround. F241 resolves aggregate disclosure toward the server because the client cannot be a second authority for visibility. The server must independently derive the readable filtered cohort and the unfiltered comparison cohort before calling `applyDisclosureControl`; trusting client-selected IDs would let a device choose the disclosure cohort and would leave no defined meaning for the unfiltered set.

**Status: open.** Stage 13 is blocked before implementation. A ruling must define one shared filter input schema and matching semantics for both the local row query and the server aggregate, including skill/entity traversal, AND/OR behavior, and availability-window meaning. `VRS-F005` and Stage 13 items 3–5 must then carry that contract directly.
