...
Stage 15 contract tracing added open F250 before product code was written: **196 numbered rows, F54–F250 with one gap (F198): 181 closed, nine open (F70, F71, F73, F85, F91, F118, F125, F129, F250), and six recorded facts or limitations (unchanged).**
...
**Decision required.** The established precedent strongly suggests versioned contracts — `ghostResource.cancel(ghostId, expectedVersion)` and `ghostResource.promote(ghostId, expectedVersion, details)` — with the version checked first, plus serializable execution for promotion so only one transaction can commit. The ruling must also define the exact rejection semantics for an already-promoted Ghost whose supplied version is stale, and require the optimistic `ghostResource.cancel` handler to perform the same version check if cancellation remains offline-capable. This section records that likely shape as evidence, not as a decision.
