...
| F187 | A003's "converge within 30 seconds" acceptance criterion is demonstrated (sub-second on the real stack) but not asserted as a deadline by FDN-51 Stage 4b's tests, which use 30–45 s flake-margin timeouts; the Stage 4b completion comment overstated this as "asserted" | `VPS-A003`, FDN-51 | **Open — record correction + recorded limitation, non-blocking.** A hard wall-clock deadline belongs in load/perf or CI-matrix coverage, not a functional integration test against a real relay + real PostgreSQL. Correct the completion-comment claim; a bounded-latency assertion is deferred (candidate for FDN-54 / FDN-55). Does not block the FDN-51 merge |
| F188 | `A003-T10` and the `A003-T36` prose require "a single shared core … the same core drives … the WASM client," but `A003-T42` and the build have the Rust `[lib]` plus a hand-written TypeScript wire-codec mirror the browser actually uses, synchronized by shared vectors | `VPS-A003`, FDN-51 | **Open — specification-consistency correction, non-blocking on the code.** The code is `A003-T42`-compliant and interoperability is proven by the shared vectors. Amend `A003-T10` / `T36` prose to describe the Rust `[lib]` + vector-synchronized TypeScript mirror (additive, like the Stage 5 `A003-T09` note); trim or mark the unused wire `pending_changes` enum value (post-ruling-4 drift). Recommended to land before the FDN-51 merge so branch and spec agree |


**One hundred and thirty-five numbered rows: one hundred and thirteen closed, nineteen open, and three recorded facts or limitations.** F187 and F188 were added on 8 September 2026 during FDN-51 Stage 6 (the closure audit): F187 records that the A003 30-second convergence bound is demonstrated but not asserted as a deadline (a record correction plus a recorded limitation, non-blocking); F188 records that `A003-T10` / `T36`'s "single shared core" prose contradicts `A003-T42` and the build's Rust `[lib]`-plus-synchronized-TypeScript-mirror wire codec (a specification-consistency correction, non-blocking on the code). Neither blocks the FDN-51 branch merge. F186 was added on 8 September 2026 during FDN-51 Stage 0 as a genuine open gap — the relay proves tier-level opacity but not per-recipient delivery filtering — scoped and deferred to FDN-89 by founder ruling rather than counted as a recorded fact. F185 was added the same day as a recorded design boundary — `recoverPartition()`'s scope to an already-materialized partition — counted like F76 and F157 rather than as open or closed. F167 and F184 closed by founder ruling on 8 September 2026 — A003-T14's SLIP-0039 requirement is amended to the M-of-N property it was selected to provide, with `shamir-secret-sharing@0.0.3` (SRI-pinned) as the approved primitive; F167 is no longer a SLIP-0039-related blocker of any kind. F172–F175 and the two additional adversarial findings F180–F181 remain closed by direct repository proof. Existing unrelated open findings remain unchanged.
