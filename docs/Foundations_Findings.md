...
**Decision needed.** Stage 9 point 3 says exactly one membership is "treated as active" client-side, but does not specify a server write to `session.activeOrganizationId` or a change to the guards that require it. F217 deliberately avoided silent switching when a second workspace appears, and F219 deliberately made its new route a narrow read returning only an array. Plausible repairs have different contracts: an explicit session-only activation command restricted to the sole active confirmed membership; an atomic conditional claim update in the list route; or changing the API's reliance on the active-session claim for this case. Choosing one changes the session and authorization boundary and must be ruled in `VPS-F001` and Stage 9 point 3 before building.

**Status: open.** Stage 9 stops on this same branch; the F219 work and existing preliminary bootstrap are preserved.
