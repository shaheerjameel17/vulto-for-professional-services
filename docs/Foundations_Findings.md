...
| F203 | Stage 1 of the server-authoritative build requires the CI Postgres service to run with `wal_level=logical`, but the slow lane starts Postgres as a GitHub Actions service container, which accepts an image, environment and `options` (arguments to `docker create`, placed before the image) and no container command, so the server flags cannot be passed as written | `VPS-A007`, Repository | **Open — blocks the CI mirror of the compose change.** Nothing in CI needs logical replication until Electric is exercised in CI (Stage 6 and 7), so the slow lane is unchanged. The decision is the founder's: start Postgres as a `docker run` step with the flags, use an `--entrypoint` override in `options`, or run the Electric-dependent suites from a compose-based job |
...
**What was not done.** No workaround was chosen. Each candidate changes how CI starts its database, which is a pipeline decision: replacing the service container with a `docker run` step that passes the flags, overriding the entrypoint through `options`, or running the Electric-dependent suites from a job that uses the compose file directly. The slow lane is left as it was.
