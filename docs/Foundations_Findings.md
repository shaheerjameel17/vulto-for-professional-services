...
### F217 — the ruled shell bootstrap has no authoritative active workspace to open

Raised 22 September 2026 while checking the new Stage 9 brief against the live workspace lifecycle, before writing code. F216's ruling says a resolved Better Auth session supplies `session.activeOrganizationId`, which the shell uses with `user.id` to mount `createGraphClient`. Adding `organizationClient()` exposes the session field to TypeScript, but it does not populate the field.

**The online creation path leaves it null.** `services/api/src/auth/workspace-projection.ts`'s `createWorkspace` calls `createPendingWorkspaceAdmission` and `confirmWorkspaceAdmission`, then returns the new IDs. Neither function writes the session. A search of production `services/api/src` code for `activeOrganizationId` writes finds only `workspace-session.ts`'s revocation path, which sets it to `null`. The column in `auth/schema.ts` is nullable. The existing workspace-creation integration test confirms the membership and graph records but calls `requireCurrentWorkspaceSession` with the new ID explicitly; it does not assert that the session's active organization was set. A newly created workspace therefore does not provide the value the Stage 9 shell is instructed to read. A person can also have a session with no active organization after revocation.

**The offline path has no unique active workspace either.** Stage 6 keeps a separate SQLite cache named `vulto:<workspaceId>:<userId>` for each pair; `session_hint` inside each cache repeats that pair. `listIdbDatabases` can enumerate caches, but neither the cache schema nor the device-identity database records which one was the active workspace when the device went offline. The latter deliberately permits only a device ID and pending erasure names, never person or workspace state. If a person has used two workspaces, choosing a cache by enumeration order would silently open a workspace the session had not selected. Refusing to choose would fail `VPS-A003`'s offline cold-boot guarantee for that person.

**Decision needed.** Define when and by whom `activeOrganizationId` is set for a newly created or entered workspace, what a valid session with a null active organization does at the shell, and how an offline cold start identifies the last active workspace among multiple cached workspaces without weakening sign-out or revocation erasure. This belongs to `VPS-F001`'s session/workspace contract and `VPS-A003`'s offline-boot contract. The Stage 9 builder must not invent a workspace selection or persisted marker as a micro-decision.

**State:** Open. FDN-115 and the new Stage 9 are blocked on this ruling. No shell bootstrap or browser suite was built.
