...
| F239 | `VRS-F005` G02 and its acceptance criteria require every derived bench period to exclude days carrying Pitch-categorized time, but Stage 13's local Bench Forecast query contract omits `TimesheetEntry`; the repository has no TimesheetEntry field schema or write path because `VRS-F009` and `VRS-F010` are later in the build order | `VRS-F005`, `VRS-F009`, `VRS-F010`, Stage 13 brief | **Open.** Stage 13 cannot both ship the required bench computation and follow its nine-item brief without a ruling: either explicitly defer the Pitch-day subtraction until the taxonomy and TimesheetEntry exist, or define the forward-compatible local query input and ownership boundary Stage 13 must build now |
...
**One hundred and eighty-five numbered rows, F54–F239 with one gap (F198): one hundred and seventy closed, nine open (F70, F71, F73, F85, F91, F118, F125, F129, F239), and six recorded facts or limitations (unchanged).** Stage 13 contract tracing added F239 before implementation: the required Pitch-day exclusion has no Stage 13 query contract or built TimesheetEntry schema because its owning features come later in the MVP order.
...
Stage 13 Do item 3 instead defines the local query over `Assignment`, `Project`, `Client`, `GhostResource`, `OpenRole`, `Entity`, `WorkingCalendar`, `Holiday` and `WorkingPattern`. It does not include `TimesheetEntry`, the only record that can say a person logged Pitch time. Its Done criteria likewise require offline bars, bench periods and day counts but never prove the Pitch exclusion. This is an observable correctness difference, not missing test detail: following the nine Do items produces an amber bench day that the feature's own formula explicitly excludes.

**Why implementation cannot fill it in mechanically.** `VRS-F009` owns `time_category` and `VRS-F010` owns TimesheetEntry's complete schema and write path; both come later in the MVP order. The registry contains `TimesheetEntry` and its edges ahead of implementation, but `packages/schema` has no field schema, named mutation or typed local query contract for one today. Stage 13 could either ignore Pitch days until those features exist, or introduce a forward-compatible raw-row dependency now, but those choices establish different acceptance scope and ownership. The brief author explicitly enumerated every local node type the query reads, so silently adding another future-owned type would exceed the ruled contract just as silently omitting it would violate G02.

**Ruling required.** Decide one of these boundaries and correct both `VRS-F005` and the Stage 13 brief to match:

1. Stage 13 derives bench periods from working days and Assignment coverage only, with Pitch-day subtraction explicitly deferred to the stage that builds `VRS-F009`/`VRS-F010`; or
2. Stage 13 defines and tests the minimal forward-compatible TimesheetEntry read contract needed for Pitch-day subtraction now, without taking ownership of the later feature's mutation or full schema.

**Status: open.** Stage 13 is blocked before implementation; no product code was changed.
