...
| F204 | The Stage 2 brief defines `graph_nodes.workspace_id` as `not null` with `node_id` as the primary key, but `VPS-A002`'s Universal node conventions say a `User` node carries no `workspace_id`, because one person may hold membership in several workspaces; one `User` row cannot belong to a workspace and cannot be duplicated per workspace under a single `node_id` key | `VPS-A002`, `VPS-A003`, Stage 2 brief | **Open — blocks Stage 2.** The database column, its indexes, the store's `insertNode` for `User`, and the founding `User` record in `workspace.create` all depend on the answer. Recommended: allow `workspace_id` to be null for `User` rows only (enforced by a check constraint), and let the sync audience alone scope which `User` rows a device holds |
...
**Recommendation.** Allow `workspace_id` to be null for `User` rows only, enforced by a check constraint (`workspace_id is not null or node_type = 'User'`), and have the Workspace node's equal its own `node_id`. A `User` row is then one row per person, as `VPS-A002` describes. Scoping of `User` rows to devices comes entirely from the sync audience, so Stage 6's node shape drops its `workspace_id = $1` condition and relies on the audience subquery, which is already keyed by workspace and person. `VPS-A003`'s canonical store table gains one sentence saying so.
