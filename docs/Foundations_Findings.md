...
| F184 | `privy-io/shamir-secret-sharing` is a genuine, twice-audited (k,n) Shamir implementation that satisfies A003-T13/T14's stated purpose without SLIP-0039's transcribable-share format F183 ruled unnecessary, and without the pairwise-envelope scheme's combinatorial limit | `VPS-A003`, Linear | **Open, pending founder review of the two source audit reports** — the technical case is strong; I could not extract the PDF report text myself to confirm scope/date/remediation firsthand |
...
### F184 — a genuine, twice-audited Shamir library resolves F167's actual requirement without SLIP-0039's format or the pairwise scheme's blowup

Raised assessing `privy-io/shamir-secret-sharing` as a direct F167 candidate, once F183 was decided (no requirement that Tier 1 recovery be portable outside Vulto's own software).

**What it is, verified from source and the registry rather than from its own README's summary line.** `shamir-secret-sharing`, published by Privy, Apache-2.0, zero runtime dependencies. The polynomial arithmetic in `src/index.ts` is textbook GF(2⁸) Shamir secret sharing using lookup tables sourced from HashiCorp Vault's own implementation — a standard, well-known construction, not an approximation and not bespoke math. `split(secret, shares, threshold)` and `combine(shares)` operate on arbitrary-length `Uint8Array` byte strings, which fits a 256-bit secret directly. The browser build's randomness (`src/csprng.ts`) calls `crypto.getRandomValues` — the standard Web Crypto API already used everywhere else in Vulto's cryptographic Worker, with no Node-only API and no WASM/Go toolchain required. Below the M-of-N threshold, the scheme is information-theoretically secure — this is genuine Shamir, not an engineered lookalike.

**The audit history is unusually good, and unusually honest about a real mistake.** Cure53 audited the pre-release implementation in February 2023 and found `PVY-01-002`: the leading polynomial coefficient could be zero, theoretically reducing the effective degree. Privy fixed it by forcing that coefficient non-zero. A second firm, Zellic — joined by informal input from a third, Cypher Stack — later identified that the *fix itself* was the more subtle defect: forcing one coefficient non-zero breaks the uniform-random-sampling requirement the security proof depends on. Cure53 reviewed this finding and **publicly reversed its own original recommendation** in April 2024, agreeing that uniform sampling is the more correct construction. The code was reverted to uniform sampling in commit `3333451`, tagged `v0.0.4` and published to npm on 10 January 2025 (`gitHead` on the npm registry matches the GitHub tag exactly). Comparing `v0.0.4` against the current `main` branch shows zero changes to `src/index.ts` since that revert — the only difference is a documentation wording update in April 2026. The shipped, pinnable artifact is the one two firms converged on, not an unreviewed accumulation since the last report.

**Quantified for our exact case, per founder instruction, after an earlier draft of this entry stated the wrong probability.** For a 256-bit Tier 1 recovery secret under the 2-of-3 default, each byte is shared by a degree-1 polynomial, `f(x) = b + c·x`, with exactly one random coefficient `c` drawn uniformly over GF(2⁸)'s 256 values. `c = 0` — the degenerate case — has probability exactly **1/256 per byte**. Across the 32 independent bytes: **P(at least one of 32 bytes degenerate) = 1 − (255/256)³² ≈ 11.77%, roughly 1 split in 8.5.** Expected degenerate bytes per split: 0.125. An earlier version of this paragraph stated "~2⁻²⁵⁶-probability all-zero case," which is the probability of every one of the 32 bytes being simultaneously zero — a different, essentially irrelevant event. The correct per-split figure is ~11.8%, not astronomically small, and is stated as such rather than folded into the smaller number.

**The impact, not just the probability, is what makes this acceptable — and it is a structural argument, not a magnitude argument.** At the 2-of-3 default, "below threshold" means exactly one share. One share yields one equation, `y = b + c·x`, in two unknowns, satisfied by *every* possible secret byte `b` for an appropriately chosen `c` — including when the true `c` is 0. Telling the degenerate case apart from the general one requires comparing **two** points for that byte (checking `y₁ = y₂`), and two shares is already the complete 2-of-3 threshold. So the event this history describes cannot be detected, let alone exploited, by anyone who does not already hold full authorized access — this is the standard Shamir perfect-secrecy proof (Lagrange interpolation: any threshold-minus-one points are consistent with every possible secret, for some choice of the unknown coefficients), and it holds independent of which coefficients were actually realized, not merely on average.

Checked against the literature rather than resting on this derivation alone: **Ghodosi et al.'s actual finding, confirmed independently of Privy's own account, is that forcing coefficients non-zero — the construction that was reverted — is what lets a below-threshold holder statistically rule out candidate secret values**, because some (secret, coefficient) pairs become structurally impossible under that restriction; exploiting it needs many splits (or, per the same body of work, structural weakness in specific polynomial classes at thresholds of 5 or 6 and above, unrelated to this question and well above our default). **The uniform-sampling construction — the one that ships in `v0.0.4`, the version this finding proposes pinning — is the version that closed this exposure, not the one that has it.** The residual concern this audit history is actually about does not apply to the artifact being recommended.

**Vulto's own architecture forecloses any residual value of a partial byte even in the branch where the event occurs, without adding anything new.** The reconstructed secret is HKDF input key material for an AES-256-GCM key that must authenticate against the RFC 8785 canonical header. A wrong reconstruction fails authentication outright — binary, no partial credit — and HKDF mixes all 32 bytes non-linearly before producing the derived key, so even certain knowledge of one byte gives no way to test that knowledge byte-by-byte against the AEAD tag. The application-level check the founder's instruction raised as a possible addition already exists as a side effect of the design already in place; nothing new is needed on that account.

**What I could not verify myself, stated plainly rather than glossed.** Both audit reports are PDFs (Cure53's own site; Zellic's GitHub publications repository), and the tools available to me could not extract their text — I have the finding dates, the finding IDs, and the resolution narrative from Privy's own engineering blog post and the public commit history, not from reading the reports' own scope and severity tables directly. That is a real gap between "well-sourced secondhand account" and "verified firsthand," and it is the reason this is not recorded as closed. Reading both PDFs directly is a same-day task, not an audit engagement — nothing like go-slip39's multi-month commissioning gate.
...
**The library's one stated limitation is already covered by Vulto's own design, not left as a residual gap.** Its README states it "does not verify the result of share reconstruction" — a caller feeding in wrong or corrupted shares can get back a wrong value with no signal. Vulto's architecture already wraps everything downstream in authenticated encryption: a wrongly reconstructed secret used as HKDF input key material produces a key that fails to authenticate the AES-GCM ciphertext it is asked to decrypt. The failure mode is "recovery visibly fails," not "recovery silently succeeds with a wrong key" — the same property every other envelope in A003 already has.
...
**Proposed A003-T14 amendment, for review rather than applied:** replace the SLIP-0039 naming with the property A003 actually needs — an M-of-N threshold scheme over a workspace recovery secret, implemented by a pinned, audited, standards-form Shamir secret-sharing library — and name the specific pinned artifact (`shamir-secret-sharing`, Privy, `v0.0.4`) in the cryptographic-suite table alongside the fixed primitives it feeds. The amendment should state explicitly that this trades away hardware-wallet/cross-vendor share portability, and that F183 already ruled that trade acceptable.

**One design note worth carrying into the amendment, costing nothing.** The statistical-bias question the audit history surfaced only matters across many splits of an *identical* secret. Vulto's own construction should generate a fresh 256-bit recovery secret at every split (each Tier 1 holder-set change already rotates the epoch and re-wraps keys) rather than ever re-splitting the same secret twice — a rule the existing design already implies and costs nothing to state outright.

**Open, pending founder review of the two source audit reports — unchanged by the zero-coefficient quantification above.** That analysis makes the technical case stronger, not weaker: the version proposed for pinning is specifically the version that closed the one real exposure this history contains. It does not substitute for reading the audits themselves, which remains the reason this stays open rather than closed on my own authority.
