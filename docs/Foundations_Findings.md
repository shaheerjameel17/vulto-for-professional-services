...
**Decision required.** The ruling must define the canonical Ghost-time record for every required Employee field and the promotion-time path that leaves a valid real Employee. It may introduce a Ghost-specific discriminated schema/record constructor with explicit nullability, add the missing identity/work-pattern fields to creation or promotion, or define safe derived placeholders and their replacement rules; the current documents choose none of these. In particular, the workspace-unique `employee_code` and non-FullTime `contracted_hours` cannot be guessed in code.

**Status: open — Stage 15 is blocked.**
