...
| F259 | `VRS-F008` requires `conflictCheck.evaluate` to run from the local graph within 100ms and says computation and override function fully offline, while Stage 16 requires a server-side read query and marks `overrideAndProceed` `onlineOnly` with no optimistic handler | `VRS-F008` NFRs/API contracts, Stage 16 Do items 4/9, `VPS-A003` | **Open.** The stage cannot satisfy both the feature's local/offline contract and the corrected brief's server-confirmed contract. A ruling must choose the authoritative execution boundary and correct the other document before implementation |
