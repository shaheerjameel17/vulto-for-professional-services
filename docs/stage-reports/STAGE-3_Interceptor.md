...
- [x] Support and system principals are evaluated and audited — evidence: `interceptor.integration.test.ts` `support principals (F206)` (7 tests: scope cap, never above Owner, Tier 3 never, the four forbidden writes, read-write gating, audit shape) and `system principals (F206)` (2 tests); `packages/schema/src/audit.test.ts::F206 — the audit actor` (3 tests). Formerly: see F206.
...
| A004-T22 (principals evaluated and audited) | `permission/principal.ts`, `permission/interceptor.ts`, `packages/schema/src/policy/principal-policy.ts`, `packages/schema/src/audit.ts` | `interceptor.integration.test.ts` support and system describes; `audit.test.ts::F206 — the audit actor` |
| F206 (audit actor shape, support scope) | `audit/schema.ts`, migration `0013_milky_ravenous.sql`, `VPS-F004` G08 | `interceptor.integration.test.ts::produces correctly shaped audit entries: the grant, no person, no roles` |
...
```
 ...
 docs/stage-reports/STAGE-3_Interceptor.md          |  139 ++
 ...
 49 files changed, 7488 insertions(+), 903 deletions(-)
```

## 5. Database changes
Migration `services/api/drizzle/0012_wonderful_morg.sql`: creates `audit_journal` (`workspace_id` references `organization` with cascade, `audit_entry_id`, `content_digest`, `entry jsonb`, `occurred_at`, `actor_user_id`, `event_type`, `operation`, `outcome`, `target_kind`, `target_node_type`, `target_tier`, `appended_at`), primary key `(workspace_id, audit_entry_id)`, six indexes, and check constraints on event type, outcome, target kind and target tier. Its content is the two migrations from `fdn-68-audit-wip` (`0010_nosy_zzzax`, `0011_wild_next_avengers`) combined: the second only widened one check constraint, and the generated SQL matches the WIP's final state. Migration `services/api/drizzle/0013_milky_ravenous.sql` (F206): `audit_journal.actor_user_id` becomes nullable; adds `actor_kind` (default `member`, checked), `actor_grant_id`, `actor_system_name`, an index on `(workspace_id, actor_kind, actor_grant_id, actor_system_name, occurred_at)`, and check `audit_journal_actor_identity_check` requiring exactly the identifier that matches the kind. No other tables changed.

## 6. Tests and gates
- `pnpm install --frozen-lockfile` — exit 0
- `pnpm stack:up` — exit 0
- `DATABASE_URL=postgres://vulto:vulto@localhost:5432/vulto_stage3_fresh pnpm --filter @vulto/api db:migrate` (fresh database) — exit 0, 14 migrations recorded
- `pnpm verify` — exit 0; `Tasks: 6 successful, 6 total`; `@vulto/schema` 7 files, 64 passed, 2 todo; `@vulto/graph` 23 files, 280 passed
- `pnpm verify:full` — exit 0; `@vulto/api`: `Test Files  7 passed (7)`, `Tests  140 passed (140)` (88 at the start of the stage)
...
- `decide*` functions have no side effects and are for the sync audience materializer (Stage 6); `authorize*` decide and audit. Ruled 21 September: the materializer is not audited per evaluation.
...
- Gate 2 (Client disagreement recorded as F207, closed: F008 governs) reads `OWNERSHIP_REGISTRY` and applies to node writes only. Where an owner is not one of the four activatable applications (Client, owned by Vulto Sales), no restriction applies; `VPS-A002`'s ownership table and `VPS-F008`'s policy table disagree about Client, and F008 says not applicable.
...
## 8. Findings raised
- F206 — support and system principals have no audit shape and no policy rows. **Closed** by founder-delegated decision; `VPS-F004` corrected (G08).
- F207 — `Client` write authority disagrees between `VPS-A002` and `VPS-F008`. **Closed**: F008 governs (not applicable); listed for the FDN-104 sweep.

## 9. Deviations from this brief
- None on principals. A system principal's node reads and writes are denied and audited by name; the operation list holds only `erasure`'s `audit.pseudonymize-actor` until Stages 5 and 6 add theirs.
...
Yes. Stage 4 needs the interceptor, `appendAudit` and the store. Stages 5 and 6 add their system operations to the policy table.
