...
## Right now, as of this doc's writing (26 September 2026)

Stages 21, 22 and 23 are merged (`414b8df`, `cd17db0`, `b50ab85`). **F305 is settled into a brief but still OPEN**: the sync browser suite has been red on `main` since Stage 11 (`seedEntities`/`newEntity` create a feature-owned `Entity` through `graph.createNode`), confirmed from the public Actions API (last green slow lane `250a60a`, first red `e0c05e4`, every slow-lane push since red, `publish-artifacts` skipped; and from about 24 September 20:34 UTC every job failed to start under the private-repository Actions allowance, which the founder ended by making the repository public on 26 September). **Stage 24 (`FDN-126`, the suite repair) is briefed** in `Claude_Code_Build_Prompt.md` Part 3 and `Reviewer_Handoff.md` section 25; branch `codex/stage-24-sync-browser-suite-repair` starts from `main`. `VPS-F003` (Notification and Alert Center) is Stage 25.

**Your next job is to review Stage 24's implementation when Codex reports back**, using the "What to check in the Stage 24 report" list, and to read the branch's `fast-lane` and `slow-lane` runs directly from the public Actions API (recipe under the standing verification discipline) rather than from the report. If they are green and the diff is a pure rename plus the guard, merge `--no-ff`, confirm `main`'s next slow-lane run is green with `publish-artifacts` running, then close F305 (update its table row and the recount: nine open becomes eight) and brief Stage 25 in the same sitting. Before briefing Stage 25, grep for the deferred `VPS-F003` hooks earlier stages left (for example `hrCompliance.sendReminder`, Stage 18), and verify every gate and fixture the brief names by running or reading it: five briefs in a row reached the builder with an unverified claim (F301–F305).

Linear is connected (`FDN-126` is the repair issue, Vulto Foundation, High). The repository is public; its history was scanned on 26 September for secrets and none was found, and it has no `LICENSE`, `SECURITY.md` or `CODEOWNERS` (the founder was advised).
