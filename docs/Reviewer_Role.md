...
**Binding rules: `docs/Linear_Operating_Rules.md`, added 26 September 2026 after issues were twice created in the wrong team.** Every issue MUST be created in the team that owns its feature (`VPS-*` platform work in Foundation, `VRS-*` in Roster), assigned to the founder, given a priority and its Domain and Feature Type labels; every project MUST have a lead, a priority, a phase label, an initiative, milestones and issues in those milestones. At each stage boundary the reviewer runs the audit in section 8 of that document (unassigned issues, priority 0, projects missing milestones or initiatives, issues without a milestone, wrong-team prefixes) and fixes every hit before reporting.
...
**Stage 25 (`VPS-F003`) is merged (`e52fef0`, F307 to F320 closed by ruling, F321 a recorded fact, F322 open and low; Handoff section 26). Stage 26 (`VPS-F004`) is merged (`fc7ef42`, F323 to F326 closed by ruling, no new finding; Handoff section 27). Stage 27 (`VRS-F018` part 1) is merged (F327 to F334 closed by ruling, no new finding; Handoff section 28). Next action: brief the next stage (UI-1, Inbox and command palette, FDN-131) and review it the same way; Stage 28 (TOIL) waits on the founder's decision (RST-60). The founder owes one decision before Stage 28 (where a TOIL accrual lives; Handoff section 28). Linear conventions are in Handoff section 27 (platform `VPS-*` stages belong to Foundation, `VRS-*` to Roster, each with a project and one Feature Type label).** Also pending: 92 Dependabot alerts to be handled as their own remediation stage.
