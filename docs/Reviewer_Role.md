...
Linear may or may not be connected in your session — check with `ToolSearch` for Linear tools early (query `"Linear"` alone is enough; it has come back empty for the entire history of this project so far, most recently reconfirmed 25 September 2026 — Shaheer has twice believed it connected when a fresh `ToolSearch` found nothing, so verify with a real tool call rather than taking his word or an earlier session's note for it). If connected, use it directly: create/update issues, post comments, move statuses — don't hand Shaheer copy-paste blocks for something you can just do. If not connected, fall back to giving Shaheer exact, ready-to-paste text for every issue/comment/status change, clearly labeled by target issue.
...
## Right now, as of this doc's writing (25 September 2026)

Stage 21 (`VRS-F013`, Skill-to-Project Matcher) is merged into `main` as `414b8df`. Stage 22 (`VRS-F014`, Skill Matrix) was briefed in `docs/Claude_Code_Build_Prompt.md` Part 3, and Codex has since reported it **complete, awaiting founder/reviewer review** — see `docs/stage-reports/STAGE-22_Skill_Matrix.md` (uncommitted, untracked, on branch `codex/stage-22-skill-matrix`, alongside the uncommitted implementation itself: `packages/graph/src/queries/skill-matrix.ts` and its test file, plus edits to `index.ts`/`bench-forecast.ts`/`cache-data.ts`). Nothing on that branch has been committed yet by Codex as of this writing — check `git status` before assuming otherwise, since Codex may commit or continue working between this doc being written and your session starting.

**Your first job in a fresh session is therefore the independent pre-merge verification of Stage 22** — same standing discipline as every prior stage (see above): read the actual diff/working-tree changes against the F296 ruling and the Stage 22 brief in `Claude_Code_Build_Prompt.md`, read the new test file in full, run `tsc --noEmit` plus the local `skill-matrix.test.ts`/`bench-forecast.test.ts` suite (no Postgres needed for these), and independently confirm the report's specific claims — the ranking logic in `getSkillHolders`, the `assignmentCoversDate` factoring out of `bench-forecast.ts`, the `record_json` fallback fix the report mentions finding mid-review — rather than accepting them from the report's prose. If it holds up: commit Codex's work (it is currently uncommitted on disk — coordinate with Shaheer on whether Codex will commit it or whether you should), merge `--no-ff` into `main`, document the completion review in `Reviewer_Handoff.md`, update Linear (`RST-50`, currently shown as "In Review" per the report), and brief the next stage. Do not disturb the uncommitted files by checking out or resetting that branch before you've read them.
