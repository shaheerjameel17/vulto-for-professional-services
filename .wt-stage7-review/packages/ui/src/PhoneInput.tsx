  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <div className="flex h-control items-center rounded-md border border-border-default bg-bg-surface has-[:focus-visible]:outline has-[:focus-visible]:outline-2 has-[:focus-visible]:outline-border-focus has-[:focus-visible]:outline-offset-2">
        <RadixSelect.Root value={country.code} onValueChange={changeCountry}>
          <RadixSelect.Trigger
            ...
            className="flex h-full shrink-0 items-center gap-1 rounded-l-md border-r border-border-default px-2 font-ui text-small text-text-primary hover:bg-bg-hover"
          >
              ...
              <RadixSelect.Viewport>
                {COUNTRIES.map((item) => (
                  <RadixSelect.Item
                    key={item.code}
                    value={item.code}
                    className="flex h-control cursor-pointer items-center gap-2 rounded-md px-2 font-ui text-body text-text-primary outline-none data-[highlighted]:bg-bg-hover"
                  >
      ...
      <Text variant="small" className="text-text-tertiary">
        Enter the national number only; the country code is added automatically.
      </Text>
    </div>
  );
}
